...
@dataclass(frozen=True)
class CorrelationRollupCloseout:
    source_app: str
...
def build_final_closeout(
    ...
    closeout = CorrelationRollupCloseout(
        source_app=source_app,
        ...
        deploy_allowed=False,
        handoff_summary=(
            "Correlation_ID rollup sidecar completed. "
...
def validate_final_closeout(closeout: CorrelationRollupCloseout) -> tuple[bool, tuple[str, ...]]:
    issues: list[str] = []

    if closeout.source_app != "ARCHIVE-CORRELATION-ROLLUP-APP-1":
        issues.append("invalid_source_app")

    if closeout.completed_stages != REQUIRED_COMPLETED_STAGES:
