...
class TrixPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if indicator_type not in TRIX_TYPES:
            raise ValueError("unsupported TRIX type")
        ...
        for name in ("smoothing_window", "signal_window"):
            value = getattr(self, name)
            if isinstance(value, bool) or not 2 <= value <= 2500:
                raise ValueError(f"{name} is invalid")
        if 3 * self.smoothing_window + self.signal_window - 2 > 10000:
            raise ValueError("TRIX warm-up exceeds bounded scope")
        ...
        if self.operator_registered is not True:
            raise ValueError("TRIX policy must be Operator-registered")
        if any((self.direction_label_allowed, self.threshold_or_crossover_allowed, self.score_rank_or_signal_allowed)):
            raise ValueError("TRIX policy exceeds metric-only scope")
