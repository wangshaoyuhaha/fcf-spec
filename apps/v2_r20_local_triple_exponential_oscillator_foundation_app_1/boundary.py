...
class V2R20TrixBoundary:
    ...
    def __post_init__(self) -> None:
        if not all((self.paper_only, self.local_only, self.registered_artifact_only, self.operator_review_required)):
            raise ValueError("mandatory TRIX boundary is open")
        if any((self.live_source_allowed, self.prediction_allowed, self.direction_label_allowed, self.threshold_or_crossover_allowed, self.score_rank_or_signal_allowed, self.network_access_allowed, self.account_or_execution_allowed)):
            raise ValueError("prohibited capability requested")


V2_R20_TRIX_BOUNDARY = V2R20TrixBoundary()
