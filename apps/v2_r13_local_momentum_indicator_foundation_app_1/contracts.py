...
MOMENTUM_INDICATOR_TYPES = ("RATE_OF_CHANGE", "RSI")


@dataclass(frozen=True)
class MomentumIndicatorPolicy:
    indicator_id: str
    ...
    def __post_init__(self) -> None:
        ...
        if indicator_type not in MOMENTUM_INDICATOR_TYPES:
            raise ValueError("unsupported momentum indicator type")
        ...
        if isinstance(self.window, bool) or not 2 <= self.window <= 10000:
            raise ValueError("momentum indicator window is invalid")
        ...
        if self.operator_registered is not True:
            raise ValueError("momentum policy must be Operator-registered")
        if self.score_rank_or_signal_allowed:
            raise ValueError("momentum policy exceeds calculation-only scope")
