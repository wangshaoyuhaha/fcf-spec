"""PORTFOLIO-REVIEW-D5 paper review packet."""
...
from apps.portfolio_review_app_1.review_model import build_paper_portfolio_review, validate_paper_portfolio_review
...
def build_portfolio_review_packet(
    ...
    manifest = deepcopy(source_manifest) if source_manifest is not None else build_portfolio_review_source_manifest()
    manifest_validation = validate_portfolio_review_source_manifest(manifest)
    review = build_paper_portfolio_review(candidates=candidates, source_manifest=manifest)
    review_validation = validate_paper_portfolio_review(review)
    ...
    return {
        ...
        "paper_portfolio_review": review,
        "paper_portfolio_review_valid": review_validation["valid"],
        "paper_portfolio_review_issues": review_validation["issues"],
...
def validate_portfolio_review_packet(packet: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if candidate.get("paper_portfolio_review_valid") is not True:
        issues.append("paper_portfolio_review_valid must be true")
