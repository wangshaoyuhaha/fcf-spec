...
class V2R7LocalMarketSessionBoundary:
    ...
    exchange_calendar_service_allowed: bool = False
    hardcoded_venue_schedule_allowed: bool = False
    ...
    def __post_init__(self) -> None:
        ...
        prohibited = (
            ...
            self.exchange_calendar_service_allowed,
            self.hardcoded_venue_schedule_allowed,
        ...
        if not all(value is True for value in required):
            raise ValueError("V2-R7 required authority boundary is disabled")
        if any(value is not False for value in prohibited):
            raise ValueError("V2-R7 prohibited capability is enabled")


V2_R7_LOCAL_MARKET_SESSION_BOUNDARY = V2R7LocalMarketSessionBoundary()
