...
RUNTIME_SCHEMA_VERSION = "fcf-registered-price-shape-indicator-runtime-v1"
INDICATOR_KINDS = (
...
class RegisteredPriceShapeArtifact:
    artifact_id: str
    artifact_hash: str
    byte_length: int
    rights_id: str
...
class PriceShapeRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.window, bool) or not 2 <= self.window <= 10000:
            raise ValueError("window must be an integer between 2 and 10000")
...
@dataclass(frozen=True)
class RegisteredPriceShapeSnapshot:
    artifact_id: str
    artifact_hash: str
    dataset_id: str
    dataset_version: str
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            utc(self.source_last_timestamp_utc, "source_last_timestamp_utc"),
        )
