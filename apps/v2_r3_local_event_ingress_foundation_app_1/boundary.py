...
class V2R3LocalEventIngressBoundary:
    ...
    external_queue_allowed: bool = False
    ...
    def __post_init__(self) -> None:
        ...
        if not all(value is True for value in required):
            raise ValueError("V2-R3 required authority boundary is disabled")
        if any(value is not False for value in prohibited):
            raise ValueError("V2-R3 prohibited capability is enabled")


V2_R3_LOCAL_EVENT_INGRESS_BOUNDARY = V2R3LocalEventIngressBoundary()
