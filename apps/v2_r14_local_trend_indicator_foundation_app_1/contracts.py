...
TREND_INDICATOR_TYPES = ("EMA", "MACD")


@dataclass(frozen=True)
class TrendIndicatorPolicy:
    indicator_id: str
    ...
    def __post_init__(self) -> None:
        ...
        if indicator_type not in TREND_INDICATOR_TYPES:
            raise ValueError("unsupported trend indicator type")
        ...
        if indicator_type == "MACD":
            ...
            if self.slow_window + self.signal_window - 1 > 10000:
                raise ValueError("MACD warm-up exceeds bounded scope")
        ...
        if self.operator_registered is not True:
            raise ValueError("trend policy must be Operator-registered")
        if self.score_rank_or_signal_allowed:
            raise ValueError("trend policy exceeds calculation-only scope")
