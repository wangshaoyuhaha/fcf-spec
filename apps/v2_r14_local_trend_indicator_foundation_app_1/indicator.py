...
from .contracts import TrendIndicatorPolicy
...
class TrendIndicatorEvidence:
    ...
    def __post_init__(self) -> None:
        if self.state not in {"TREND_READY", "BLOCKED"}:
            raise ValueError("invalid trend indicator evidence state")
        if self.operator_review_required is not True:
            raise ValueError("trend evidence requires Operator review")
...
def build_trend_indicator(
    ...
    *,
    ...
    reasons: list[str] = []
    ema = macd_line = signal_line = histogram = None
    required_samples = (
        policy.fast_window
        if policy.indicator_type == "EMA"
        else policy.slow_window + policy.signal_window - 1
    )
    if registry.state != "REGISTRY_READY":
        ...
        with localcontext() as context:
            context.prec = 96
            if policy.indicator_type == "EMA":
                raw_ema = _ema_sequence(values, policy.fast_window)[-1]
                ema = raw_ema.quantize(quantum, rounding=ROUND_HALF_EVEN)
            else:
                ...
                raw_macd = macd_values[-1]
                raw_signal = _ema_sequence(macd_values, policy.signal_window)[-1]
                macd_line = raw_macd.quantize(quantum, rounding=ROUND_HALF_EVEN)
                signal_line = raw_signal.quantize(quantum, rounding=ROUND_HALF_EVEN)
                histogram = (raw_macd - raw_signal).quantize(
                    quantum, rounding=ROUND_HALF_EVEN
                )
        reasons.append("REGISTERED_LOCAL_TREND_INDICATOR_READY")
    state = "TREND_READY" if reasons[-1].endswith("READY") else "BLOCKED"
    payload = {
        "ema": _text(ema),
