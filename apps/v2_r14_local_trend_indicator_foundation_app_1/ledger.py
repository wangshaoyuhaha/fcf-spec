...
class TrendIndicatorLedger:
    evidence: tuple[TrendIndicatorEvidence, ...] = ()
    capacity: int = 1000

    def __post_init__(self) -> None:
        if isinstance(self.capacity, bool) or not 1 <= self.capacity <= 10000:
            raise ValueError("trend indicator ledger capacity is invalid")
        hashes = tuple(item.evidence_hash for item in self.evidence)
        if len(set(hashes)) != len(hashes) or len(self.evidence) > self.capacity:
            raise ValueError("duplicate or excessive trend evidence")

    def append(self, item: TrendIndicatorEvidence) -> TrendIndicatorLedger:
        if len(self.evidence) >= self.capacity:
            raise ValueError("trend indicator ledger capacity exceeded")
        if any(existing.evidence_hash == item.evidence_hash for existing in self.evidence):
            raise ValueError("duplicate trend indicator evidence is prohibited")
