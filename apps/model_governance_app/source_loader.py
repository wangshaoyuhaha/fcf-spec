"""D2 read-only source loader for MODEL-GOVERNANCE-APP-1.

The loader inspects local governance source packet metadata from completed
sidecar layers. It never mutates scores, reason codes, risk flags, source
content, or core modules.
"""
...
class GovernanceSourceSpec:
    """Read-only governance source packet specification."""
...
DEFAULT_GOVERNANCE_SOURCE_SPECS: List[GovernanceSourceSpec] = [
    ...
    GovernanceSourceSpec(
        ...
        payload_type="markdown",
        governance_category="operator_review_policy",
    ),
    GovernanceSourceSpec(
        ...
        payload_type="markdown",
        governance_category="archive_integrity_policy",
    ),
    GovernanceSourceSpec(
        ...
        payload_type="markdown",
        governance_category="data_quality_policy",
    ),
    GovernanceSourceSpec(
        ...
        payload_type="markdown",
        governance_category="scenario_policy",
    ),
    GovernanceSourceSpec(
        ...
        payload_type="markdown",
        governance_category="backtest_review_policy",
    ),
    GovernanceSourceSpec(
        source_id="signal_validation_final_state",
        layer_id="SIGNAL-VALIDATION-APP-1",
        relative_path="FCF_CURRENT_STATE_SIGNAL_VALIDATION_APP_1_FINAL.md",
        payload_type="markdown",
...
def _safe_read_preview(path: Path, max_chars: int) -> Optional[str]:
    """Read a bounded text preview from a local file."""
...
def inspect_governance_source(
    repo_root: Path,
    spec: GovernanceSourceSpec,
    max_preview_chars: int = 1200,
) -> Dict[str, Any]:
    """Inspect one governance source without changing it."""

    path = repo_root / spec.relative_path
    exists = path.exists() and path.is_file()
    stat = path.stat() if exists else None
    preview = _safe_read_preview(path, max_preview_chars) if exists else None
    ...
    return {
        ...
        "governance_category": spec.governance_category,
...
def load_governance_source_manifest(
    repo_root: str | Path = ".",
    specs: Optional[Iterable[GovernanceSourceSpec]] = None,
    max_preview_chars: int = 1200,
) -> Dict[str, Any]:
    """Build a read-only source manifest for model governance."""

    root = Path(repo_root)
    selected_specs = list(specs) if specs is not None else list(DEFAULT_GOVERNANCE_SOURCE_SPECS)
    sources = [
        inspect_governance_source(root, spec, max_preview_chars=max_preview_chars)
    ...
    categories = sorted({source["governance_category"] for source in sources})

    return {
        "app_id": "MODEL-GOVERNANCE-APP-1",
        "stage_id": "MODEL-GOVERNANCE-D2",
        "loader_mode": "read_only_governance_source_metadata",
        "source_count": len(sources),
        "available_count": len(available),
        "missing_required_count": len(missing_required),
        "missing_optional_count": len(missing_optional),
        "governance_categories": categories,
...
def summarize_governance_source_manifest(manifest: Dict[str, Any]) -> Dict[str, Any]:
    """Return a compact governance loader summary."""
    ...
    return {
        ...
        "governance_category_count": len(manifest["governance_categories"]),
        "read_only": manifest["read_only"],
        "operator_review_required": manifest["operator_review_required"],
        "score_mutation_allowed": manifest["score_mutation_allowed"],
        "reason_code_mutation_allowed": manifest["reason_code_mutation_allowed"],
        "risk_flag_deletion_allowed": manifest["risk_flag_deletion_allowed"],
