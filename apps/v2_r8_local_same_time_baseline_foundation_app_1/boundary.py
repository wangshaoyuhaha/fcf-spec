...
class V2R8LocalSameTimeBaselineBoundary:
    ...
    def __post_init__(self) -> None:
        ...
        if not all(value is True for value in required):
            raise ValueError("V2-R8 required authority boundary is disabled")
        if any(value is not False for value in prohibited):
            raise ValueError("V2-R8 prohibited capability is enabled")


V2_R8_LOCAL_SAME_TIME_BASELINE_BOUNDARY = V2R8LocalSameTimeBaselineBoundary()
