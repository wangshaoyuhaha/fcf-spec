...
def resolve_btc_perpetual_funding_rule_version(
    registry: BTCPerpetualFundingMethodScheduleRegistry,
    *,
    venue_id: str,
    contract_id: str,
    at_utc: str,
) -> BTCPerpetualFundingRuleVersion:
    if not isinstance(registry, BTCPerpetualFundingMethodScheduleRegistry):
        raise TypeError("registry must be typed FCP-0048 evidence")
    ...
    if len(matches) != 1:
        raise LookupError("point-in-time funding evidence is missing or ambiguous")
    return matches[0]
