...
class V2R44BrowserGovernanceReviewEvidenceTraceBoundary:
    ...
    def __post_init__(self) -> None:
        ...
        if not all(required):
            raise ValueError("R44 evidence trace boundary must remain closed")
        if any(prohibited):
            raise ValueError("R44 prohibited capability cannot be enabled")
