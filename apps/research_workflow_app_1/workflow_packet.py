"""RESEARCH-WORKFLOW-D5 paper workflow packet."""
...
from apps.research_workflow_app_1.source_loader import build_research_workflow_source_manifest, validate_research_workflow_source_manifest
from apps.research_workflow_app_1.workflow_model import build_research_workflow_review, validate_research_workflow_review


STAGE_ID = "RESEARCH-WORKFLOW-D5"
...
def build_research_workflow_packet(
    packet_id: str,
    source_manifest: Optional[Dict[str, Any]] = None,
    generated_at_utc: Optional[str] = None,
) -> Dict[str, Any]:
    manifest = deepcopy(source_manifest) if source_manifest is not None else build_research_workflow_source_manifest()
    manifest_validation = validate_research_workflow_source_manifest(manifest)
    review = build_research_workflow_review(source_manifest=manifest)
    review_validation = validate_research_workflow_review(review)
    ...
    return {
        ...
        "research_workflow_review": review,
        "research_workflow_review_valid": review_validation["valid"],
        "research_workflow_review_issues": review_validation["issues"],
        "workflow_step_count": review["workflow_step_count"],
        "workflow_state_counts": deepcopy(review["workflow_state_counts"]),
...
def validate_research_workflow_packet(packet: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if candidate.get("research_workflow_review_valid") is not True:
        issues.append("research_workflow_review_valid must be true")
