"""MARKET-SCENARIO-D2 local source metadata loader.

The loader is paper-only and read-only.
It discovers local scenario source metadata without mutating source content.
"""
...
def default_scenario_source_specs() -> List[ScenarioSourceSpec]:
    return [
        ScenarioSourceSpec(
...
def _metadata_for_path(root_path: Path, source_kind: str, candidate_path: Path) -> ScenarioSourceMetadata:
...
def load_local_scenario_source_metadata(
    root_path: Path | str,
    specs: Optional[Iterable[ScenarioSourceSpec]] = None,
) -> List[ScenarioSourceMetadata]:
    root = Path(root_path)
    selected_specs = list(specs) if specs is not None else default_scenario_source_specs()
    records: List[ScenarioSourceMetadata] = []
    seen = set()
...
def build_scenario_source_manifest(root_path: Path | str) -> Dict[str, object]:
    records = load_local_scenario_source_metadata(root_path)
