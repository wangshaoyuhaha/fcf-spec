...
REQUIRED_ACCEPTANCE_ITEMS = (
    "release_package_reviewed",
    "validation_summary_reviewed",
    "paper_only_boundary_reviewed",
    "operator_review_recorded",
    "rollback_notes_reviewed",
    "next_stage_handoff_reviewed",
)
...
def build_paper_final_release_acceptance_gate(
    ...
    checks = {
        ...
        "release_package_paper_only": release_package_summary.get("paper_only") is True,
        "acceptance_checklist_complete": len(missing_items) == 0,
        "operator_review_required": release_package_summary.get("operator_review_required") is True,
        ...
        "no_forbidden_real_action_flags": _no_forbidden_true(release_package_summary, acceptance_checklist, operator_record),
    }
    ...
    return {
        "ok": True,
        "type": "paper_final_release_acceptance_gate",
        "acceptance_status": "accepted" if accepted else "blocked",
        "paper_final_release_accepted": accepted,
...
def summarize_paper_final_release_acceptance(gate: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(gate, dict):
        raise ValueError("gate must be a dict")

    return {
        "ok": True,
        "type": "paper_final_release_acceptance_summary",
        "status": gate.get("acceptance_status"),
        "paper_final_release_accepted": gate.get("paper_final_release_accepted") is True,
