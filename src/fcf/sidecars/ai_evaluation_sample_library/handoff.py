"""Final handoff for the local AI evaluation sample library."""
...
HANDOFF_STAGE_ID = "AI-EVALUATION-SAMPLE-LIBRARY-D6"
...
COMPLETED_STAGES = (
    "AI-EVALUATION-SAMPLE-LIBRARY-D1",
    "AI-EVALUATION-SAMPLE-LIBRARY-D2",
    "AI-EVALUATION-SAMPLE-LIBRARY-D3",
    "AI-EVALUATION-SAMPLE-LIBRARY-D4",
    "AI-EVALUATION-SAMPLE-LIBRARY-D5",
    "AI-EVALUATION-SAMPLE-LIBRARY-D6",
)
...
def build_evaluation_sample_final_handoff(
    *,
    handoff_id: str,
    review_packet: Mapping[str, Any],
    created_at_utc: str,
) -> dict[str, Any]:
    """Build the final paper-only evaluation library handoff."""

    review_errors = validate_evaluation_sample_review_packet(
        review_packet
    )
    ...
    raw_sample_keys = review_packet.get("sample_keys", [])

    sample_keys = (
        list(raw_sample_keys)
        if isinstance(raw_sample_keys, list)
        else []
    )

    summary_by_status = {
        "READY_FOR_OPERATOR_REVIEW": (
            "Evaluation sample library is structurally complete "
            "and remains subject to operator review."
        ),
        "REVIEW_REQUIRED": (
            "Evaluation sample library requires operator review "
            "before any governance acceptance."
        ),
        "BLOCKED": (
            "Evaluation sample library handoff is blocked until "
    ...
    return {
        ...
        "source_registry_id": review_packet.get(
            "source_registry_id"
        ),
        "source_coverage_status": review_packet.get(
            "coverage_status"
        ),
...
def validate_evaluation_sample_final_handoff(
    handoff: Mapping[str, Any],
) -> list[str]:
    ...
    sample_count = handoff.get("sample_count")
    ...
    sample_keys = handoff.get("sample_keys")
    ...
    if completed_stages != list(COMPLETED_STAGES):
        errors.append("completed_stages_mismatch")

    if not _valid_string_list(
        handoff.get("source_validation_errors")
    ):
        errors.append("source_validation_errors_invalid")
    ...
    if handoff.get("source_coverage_status") not in (
        "PASS",
        "REVIEW_REQUIRED",
        "FAIL",
    ):
        errors.append("source_coverage_status_invalid")
