"""Final handoff for VALIDATION-BASELINE-D6."""
...
def build_validation_baseline_final_handoff(
    *,
    baseline_packet: Mapping[str, Any],
) -> Dict[str, Any]:
    """Build final read-only validation baseline handoff."""

    if not isinstance(baseline_packet, Mapping):
        raise ValueError("baseline_packet is required")

    status = baseline_packet.get("baseline_packet_status", "UNRESOLVED")
    ...
    return {
        ...
        "stage": "D6",
        "handoff_type": "validation_baseline_registry_final_handoff",
        "baseline_handoff_status": status,
        "source_packet": deepcopy(dict(baseline_packet)),
...
def classify_validation_baseline_final_handoff(
    handoff: Mapping[str, Any],
) -> Dict[str, Any]:
    """Classify final handoff without fabrication, repair, or execution."""

    status = handoff.get("baseline_handoff_status", "UNRESOLVED")
