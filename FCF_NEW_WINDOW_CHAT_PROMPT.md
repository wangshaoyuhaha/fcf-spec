...
??????????????CONTROL-CENTER-V2-AI-INTELLIGENCE-LAYER-SPEC-APP-1

??? FCF / BTC finance platform ?????????????????FCF?????????????????????????????CF V2 = ??????????????+ ??? AI ??????????AI ?????????????????????????????????????????????????????????????????????Dashboard Scanner???????????????????AI-CONTEXT-EVIDENCE-CONTRACT-APP-1??????????I evidence contract -> contrarian challenge -> dashboard contradiction scanner -> market narrative context -> scenario simulation -> orchestration roadmap????? core frozen??idecar-only??aper-only??ocal-only??ead-only??perator review required?????????????????????roker/exchange API??PI key??allet private key??eal account??eal position??uy/sell/order?????????????????????????????ag??elease??eploy??
---

??????????????CONTROL-CENTER-V2-AI-HARDENING-GAPS-APP-1

??? FCF / BTC finance platform ???????????????????????????? AI-CONTEXT-EVIDENCE-CONTRACT-APP-1 ????????? V2 AI ???????????????????I ???????????I ???????????rompt / model version governance??hallenge AI ??????????uman Review ???????I ???????????I ???????????? AI schema ???????????????????2 sidecar ?????contract????????????????rtifact??andoff??inal current state????? core frozen??idecar-only??aper-only??ocal-only??ead-only??perator review required??
---

??????????????CONTROL-CENTER-V2-AI-DELIVERY-FOUNDATION-APP-1

??? FCF / BTC finance platform ???????????????????????????? AI-CONTEXT-EVIDENCE-CONTRACT-APP-1 ????????? 6 ??V2 AI ??????????????????DR ???????????I ??????????esearch Artifact Package ?????uman Override Ledger??I ?????????????????????????????????2 sidecar ?????contract????????????????????????????rtifact??andoff??inal current state????? core frozen??idecar-only??aper-only??ocal-only??ead-only??perator review required??
---

??????????????CONTROL-CENTER-V2-AI-RUNTIME-OPERATIONS-GUARD-APP-1

??? FCF / BTC finance platform ?????????????????????????CONTROL-CENTER-V2-AI-RUNTIME-OPERATIONS-GUARD-APP-1 ?????V2 AI ??????????????????????????????ource Trust Level??esearch_run_id??I ???/???/???????????????????olden Path Demo??top Rule / Freeze Rule?????????????? AI-CONTEXT-EVIDENCE-CONTRACT-APP-1???????????????????? core frozen??idecar-only??aper-only??ocal-only??ead-only??perator review required??
...
<!-- MARKET-NARRATIVE-CONTEXT-APP-1-FINAL-PROMPT-SYNC -->
...
Do not execute any earlier start-D1, active-phase, approved-next-phase,
or planning-candidate instruction automatically.
...
<!-- AI-ORCHESTRATION-ROADMAP-APP-1-FINAL-PROMPT-SYNC -->
...
Do not automatically implement runtime AI orchestration.
...
<!-- AI-CAUSAL-REASONING-CHAIN-APP-1-FINAL-PROMPT-SYNC -->

## NEW WINDOW LATEST STATE OVERRIDE

This is the highest-priority current-state instruction.
...
- registered causal claim and premise structure
- registered supporting, counter, and alternative evidence
- deterministic causal-chain construction
- structural and evidence-gap assessment
- governance review packet
- operator-review and manual archive handoff
...
Do not automatically start comprehensive report synthesis or runtime
AI orchestration.

Response requirements:
- reply in Chinese
- provide complete copyable PowerShell
- use ASCII English in repository files
- do not require manual file editing
- write logs to Desktop
- do not use exit
- report commit, push, validation, and git status
...
Required order: D1 launch contracts; D2 registered demonstrative starter
package; D3 guided loopback launch and explicit browser opening; D4 startup
diagnostics; D5 Operator usability acceptance; D6 closeout and merge.

Permanent boundary: P1-P47 frozen; no P48; paper-only; local-only;
loopback-only; registered-artifact-only; read-only; Operator review required;
Deterministic Engine and Registered Evidence authority preserved; AI advisory
only; no external data, public exposure, credentials, account, wallet, order,
execution, automatic learning, tag, release, or deploy.
...
Delivery: D1 af31949dc4673d2be6aeb6b3807639b478bed46e; D2
2c707d342e8f639c1f2175d70d6366cbaf5badeb; D2 integrity fix
c996f66bc4fa82c7a0e8ee48c53b8f3b26531a3b; D3
312b2ebf04a88ec6c11089c3e57f2cf473b6f12b; D4
a5ce0862115dd67dabb771c484704f234eb09404; D5
4c2ca5f3e313e1b9fd9a369e3c03bf85a3475253; D6
7083f16e7a1bb030f03f09b63f53c9fc7a110f83.

Main merge: 2cfe860ac8c9847819557da1fbb405b7e3952eaa

Validation: 520 targeted tests passed with 3 skipped; 4266 full tests passed
with 3 skipped; scripts/run_all_checks.py PASSED; generated outputs RESTORED.

The local read-only Browser Product Console can now be started from the
Windows double-click launcher. Its default registered starter artifacts are
DEMONSTRATION_ONLY examples for A-share, US equity, and BTC. Browser opening
requires explicit Operator invocation. The product remains paper-only,
local-only, loopback-only, sidecar-only, registered-artifact-only, and
read-only. Operator review is mandatory. Deterministic Engine and Registered
Evidence remain authoritative; AI remains advisory only. No broker, exchange,
credentials, account, balance, position, wallet, order, real execution, tag,
release, or deploy path exists.

Next phase: NOT_SELECTED. NOT_APPROVED. Do not start automatically.
...
Deliver D1-D6 only: immutable lifecycle/state contracts; deterministic
preflight and migration checks; single-instance background start, graceful stop
and status; health/readiness/browser/diagnostics; bounded backup, upgrade
snapshot, recovery staging, rollback preparation, state export and Windows
double-click entry points; acceptance and authority sync.

Normal use must not require typed Python or PowerShell. Operations remain exact
loopback and may not terminate unrelated processes. Recovery never overwrites
active authority automatically.

P1-P47 remain frozen; no P48. Keep permanent paper-only, local-only,
loopback-only, sidecar-only, registered-artifact-only and Operator-review
boundaries. No live retrieval, model invocation, Prompt execution, credentials,
accounts, wallets, brokers, exchanges, balances, positions, orders, execution,
...
delivery `707820ca3d6dd8ddd8505da4d0b558bbc724ccd5`, merge
`3fe92c21f0eaf6daf079fa003e84baae0b3ae8d9`.

Validation: 17 independent; 177 targeted; 4555 full passed, 5 skipped;
run_all_checks PASSED; generated outputs RESTORED; diff check PASSED.

Delivered six-market Paper/Shadow validation and Web Console integration.
