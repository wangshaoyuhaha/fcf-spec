...
DELIVERY_ID = "FCF-FCP-0067-BTC-PERPETUAL-PAPER-STRESS-EVALUATION-MEASURE-FORMULA-SEMANTICS-REGISTRY-APP-1"
MARKER = "FCP 0067 BTC PERPETUAL PAPER STRESS EVALUATION MEASURE FORMULA SEMANTICS REGISTRY APP 1"
...
def build_fcp_0067_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next((item for item in intake.get("proposals", []) if item.get("proposal_id") == "FCF-FCP-0067"), {})
    expected = {
        "FCF_CURRENT_STATE_FCP_0067_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_MEASURE_FORMULA_SEMANTICS_REGISTRY_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0067_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_MEASURE_FORMULA_SEMANTICS_REGISTRY_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0067_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_MEASURE_FORMULA_SEMANTICS_REGISTRY_APP_1_FINAL.md",
        "docs/FCF_FCP_0067_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_MEASURE_FORMULA_SEMANTICS_REGISTRY_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "final_evidence_when_closed": not closed or ((root / "FCF_CURRENT_STATE_FCP_0067_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_MEASURE_FORMULA_SEMANTICS_REGISTRY_APP_1_FINAL.md").is_file() and all(finals) and "ALL CHECKS PASSED" in finals[0]),
        ...
        "architecture_registered": all(term in architecture for term in ("## 100. BTC Perpetual Paper Stress Evaluation Measure Formula Semantics Registry", "absolute relative difference", "nonpositive baseline")),
        "adr_registered": all(term in adr for term in ("FCF-V2-ADR-067", "Register Closed Stress Measure Formulas Before Evaluation", "zero denominator")),
        "gaps_preserved": all(term in gaps for term in ("V2-FR-GAP-098", "V2-FR-GAP-099", "V2-FR-GAP-100", "V2-FR-GAP-101", "RESEARCH_REQUIRED")),
        "protocol_registered": "Proposal `FCF-FCP-0067`" in protocol,
        "memory_registered": all(term in memory for term in ("measure-formula semantics registries", "exact typed FCP-0066 lineage", "evaluation or calculation authority")),
        "delivery_files_exist": all((root / path).is_file() for path in (
            "FCF_CURRENT_STATE_FCP_0067_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_MEASURE_FORMULA_SEMANTICS_REGISTRY_APP_1_APPROVED.md",
            "FCF_CURRENT_STATE_FCP_0067_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_MEASURE_FORMULA_SEMANTICS_REGISTRY_APP_1_DELIVERED.md",
            "docs/FCF_FCP_0067_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_MEASURE_FORMULA_SEMANTICS_REGISTRY_APP_1_D1_D6.md",
            "apps/fcp_0067_btc_perpetual_paper_stress_evaluation_measure_formula_semantics_registry_app_1/registry.py",
            "tests/fcp_0067_btc_perpetual_paper_stress_evaluation_measure_formula_semantics_registry_app_1/test_d1_d6.py",
        )),
        "run_all_wired": "control_center_fcp_0067_btc_perpetual_paper_stress_evaluation_measure_formula_semantics_registry_guard.py" in run_all,
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0067 guard failed: " + failures)
