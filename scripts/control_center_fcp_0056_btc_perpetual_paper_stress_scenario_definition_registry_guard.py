...
DELIVERY_ID = (
    "FCF-FCP-0056-BTC-PERPETUAL-PAPER-STRESS-SCENARIO-DEFINITION-REGISTRY-APP-1"
)
AUTHORITIES = tuple(
    Path(value)
    for value in (
...
MARKER = "FCP 0056 BTC PERPETUAL PAPER STRESS SCENARIO DEFINITION REGISTRY APP 1"
...
def build_fcp_0056_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0056"
        ),
        {},
    )
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0056_BTC_PERPETUAL_PAPER_STRESS_SCENARIO_DEFINITION_REGISTRY_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0056_BTC_PERPETUAL_PAPER_STRESS_SCENARIO_DEFINITION_REGISTRY_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0056_BTC_PERPETUAL_PAPER_STRESS_SCENARIO_DEFINITION_REGISTRY_APP_1_FINAL.md",
        "docs/FCF_FCP_0056_BTC_PERPETUAL_PAPER_STRESS_SCENARIO_DEFINITION_REGISTRY_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "architecture_registered": all(
            ...
            for term in (
                ...
                "price gap, thin book, venue outage, resync",
        ...
        "adr_registered": all(
            ...
            for term in (
                "FCF-V2-ADR-056",
                "Register BTC Paper Stress Definitions Before Evaluation",
                "closed scenario-kind vocabulary",
            )
        ),
        "gaps_preserved": all(
            term in gaps
        ...
        "protocol_registered": "Proposal `FCF-FCP-0056`" in protocol,
        "memory_registered": all(
            term in memory
            for term in (
                "BTC perpetual Paper stress-scenario definition registries",
                "exact FCP-0055",
                "no evaluation, account, or execution authority",
            )
        ),
        "delivery_files_exist": all(
            ...
            for path in (
                ...
                "FCF_CURRENT_STATE_FCP_0056_BTC_PERPETUAL_PAPER_STRESS_SCENARIO_DEFINITION_REGISTRY_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0056_BTC_PERPETUAL_PAPER_STRESS_SCENARIO_DEFINITION_REGISTRY_APP_1_D1_D6.md",
                "apps/fcp_0056_btc_perpetual_paper_stress_scenario_definition_registry_app_1/contracts.py",
                "tests/fcp_0056_btc_perpetual_paper_stress_scenario_definition_registry_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0056_btc_perpetual_paper_stress_scenario_definition_registry_guard.py"
            in run_all
        ),
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0056 guard failed: " + ",".join(failed))
