...
DELIVERY_ID = (
    "FCF-FCP-0042-A-SHARE-CROSS-SOURCE-OPERATOR-DELTA-REVIEW-PACKET-APP-1"
)
AUTHORITIES = tuple(
    Path(value)
    for value in (
...
MARKER = "FCP 0042 A SHARE CROSS SOURCE OPERATOR DELTA REVIEW PACKET APP 1"
...
def build_fcp_0042_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0042"
        ),
        {},
    )
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0042_A_SHARE_CROSS_SOURCE_OPERATOR_DELTA_REVIEW_PACKET_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0042_A_SHARE_CROSS_SOURCE_OPERATOR_DELTA_REVIEW_PACKET_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0042_A_SHARE_CROSS_SOURCE_OPERATOR_DELTA_REVIEW_PACKET_APP_1_FINAL.md",
        "docs/FCF_FCP_0042_A_SHARE_CROSS_SOURCE_OPERATOR_DELTA_REVIEW_PACKET_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "architecture_registered": all(
            ...
            for term in (
                ...
                "Exact parity yields an Operator",
                "assigns no severity, recommendation, threshold",
            )
        ),
        "adr_registered": all(
            term in adr
            for term in (
                "FCF-V2-ADR-042",
                "Separate Review Facts from Source Decisions",
                "call exact parity accepted automatically",
            )
        ),
        "gap_preserved": "V2-FR-GAP-109" in gaps and "RESEARCH_REQUIRED" in gaps,
        "protocol_registered": "Proposal `FCF-FCP-0042`" in protocol,
        "memory_registered": all(
            term in memory
            for term in (
                "cross-source Operator delta-review packets",
                "deterministic finding codes",
                "severity, recommendation, threshold",
            )
        ),
        "delivery_files_exist": all(
            ...
            for path in (
                ...
                "FCF_CURRENT_STATE_FCP_0042_A_SHARE_CROSS_SOURCE_OPERATOR_DELTA_REVIEW_PACKET_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0042_A_SHARE_CROSS_SOURCE_OPERATOR_DELTA_REVIEW_PACKET_APP_1_D1_D6.md",
                "apps/fcp_0042_a_share_cross_source_operator_delta_review_packet_app_1/review.py",
                "tests/fcp_0042_a_share_cross_source_operator_delta_review_packet_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0042_a_share_cross_source_operator_delta_review_packet_guard.py"
            in run_all
        ),
...
def main() -> int:
    report = build_fcp_0042_guard_report()
    if report["ok"] is not True:
        failed = sorted(key for key, value in report["checks"].items() if not value)
        raise SystemExit("FCP-0042 guard failed: " + ",".join(failed))
