...
DELIVERY_ID = "FCF-FCP-0064-BTC-PERPETUAL-PAPER-STRESS-EVALUATION-OPERAND-EVIDENCE-REGISTRY-APP-1"
AUTHORITIES = tuple(
    Path(value)
    for value in (
...
MARKER = "FCP 0064 BTC PERPETUAL PAPER STRESS EVALUATION OPERAND EVIDENCE REGISTRY APP 1"
...
def build_fcp_0064_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0064"
        ),
        {},
    )
    expected = {
        "FCF_CURRENT_STATE_FCP_0064_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_OPERAND_EVIDENCE_REGISTRY_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0064_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_OPERAND_EVIDENCE_REGISTRY_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0064_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_OPERAND_EVIDENCE_REGISTRY_APP_1_FINAL.md",
        "docs/FCF_FCP_0064_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_OPERAND_EVIDENCE_REGISTRY_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "architecture_registered": all(
            ...
            for term in (
                ...
                "one exact typed FCP-0063 operand-schema snapshot",
                "Paired baselines must precede current observations",
            )
        ),
        "adr_registered": all(
            term in adr
            for term in (
                "FCF-V2-ADR-064",
                "Require Registered Evidence For Every Stress Operand",
                "baseline observations preceding paired current observations",
            )
        ),
        "gaps_preserved": all(
            term in gaps
        ...
        "protocol_registered": "Proposal `FCF-FCP-0064`" in protocol,
        "memory_registered": all(
            term in memory
            for term in (
                "stress-evaluation operand-evidence registries",
                "FCP-0063 lineage",
                "baseline-before-current time order",
            )
        ),
        "delivery_files_exist": all(
            ...
            for path in (
                ...
                "FCF_CURRENT_STATE_FCP_0064_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_OPERAND_EVIDENCE_REGISTRY_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0064_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_OPERAND_EVIDENCE_REGISTRY_APP_1_D1_D6.md",
                "apps/fcp_0064_btc_perpetual_paper_stress_evaluation_operand_evidence_registry_app_1/registry.py",
                "tests/fcp_0064_btc_perpetual_paper_stress_evaluation_operand_evidence_registry_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0064_btc_perpetual_paper_stress_evaluation_operand_evidence_registry_guard.py"
            in run_all
        ),
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0064 guard failed: " + failures)
