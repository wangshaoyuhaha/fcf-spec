...
DELIVERY_ID = "FCF-FCP-0027-REGISTERED-DATA-PRIMITIVE-TYPE-INTEGRITY-HARDENING-APP-1"
AUTHORITIES = (
...
APPROVAL_START = "<!-- FCP 0027 REGISTERED DATA PRIMITIVE TYPE INTEGRITY HARDENING APP 1 APPROVAL START -->"
APPROVAL_END = "<!-- FCP 0027 REGISTERED DATA PRIMITIVE TYPE INTEGRITY HARDENING APP 1 APPROVAL END -->"
LOCK_START = "<!-- FCP 0027 REGISTERED DATA PRIMITIVE TYPE INTEGRITY HARDENING APP 1 LOCK START -->"
LOCK_END = "<!-- FCP 0027 REGISTERED DATA PRIMITIVE TYPE INTEGRITY HARDENING APP 1 LOCK END -->"
FINAL_START = "<!-- FCP 0027 REGISTERED DATA PRIMITIVE TYPE INTEGRITY HARDENING APP 1 FINAL START -->"
FINAL_END = "<!-- FCP 0027 REGISTERED DATA PRIMITIVE TYPE INTEGRITY HARDENING APP 1 FINAL END -->"
...
def build_fcp_0027_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
        ...
        a_substrate = btc_substrate = a_bridge = btc_bridge = ""
        readable = False
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0027"
        ),
        {},
    )
    checks = {
        ...
        "btc_digest_exact": "digest = self.content_sha256" in btc_substrate
        and "result = value" in btc_bridge,
        "integer_exact": all(
            "not isinstance(self." + name + ", int)" in text
            for text, name in (
                (a_substrate, "byte_length"),
                (a_substrate, "record_count"),
                (btc_substrate, "byte_length"),
                (btc_substrate, "source_sequence"),
                (btc_substrate, "schema_version"),
                (btc_substrate, "previous_sequence"),
                (a_bridge, "byte_length"),
                (a_bridge, "row_count"),
                (btc_bridge, "byte_length"),
            )
        ),
        "boolean_exact": "raw_repository_storage_allowed is not False" in a_bridge
        and "provider_selected is not False" in a_bridge
        and "raw_repository_storage_allowed is not False" in btc_bridge
        and "provider_selected is not False" in btc_bridge,
        ...
        "run_all_wired": "control_center_fcp_0027_registered_data_primitive_type_integrity_hardening_guard.py"
...
def main() -> int:
    report = build_fcp_0027_guard_report()
    if report["ok"] is not True:
        failed = ",".join(
            sorted(name for name, value in report["checks"].items() if not value)
        )
        raise SystemExit("FCP-0027 guard failed: " + failed)
