...
DELIVERY_ID = "FCF-FCP-0048-BTC-PERPETUAL-FUNDING-METHOD-SCHEDULE-EVIDENCE-REGISTRY-APP-1"
...
MARKER = "FCP 0048 BTC PERPETUAL FUNDING METHOD SCHEDULE EVIDENCE REGISTRY APP 1"
...
def build_fcp_0048_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next((item for item in intake.get("proposals", []) if item.get("proposal_id") == "FCF-FCP-0048"), {})
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0048_BTC_PERPETUAL_FUNDING_METHOD_SCHEDULE_EVIDENCE_REGISTRY_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0048_BTC_PERPETUAL_FUNDING_METHOD_SCHEDULE_EVIDENCE_REGISTRY_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0048_BTC_PERPETUAL_FUNDING_METHOD_SCHEDULE_EVIDENCE_REGISTRY_APP_1_FINAL.md",
        "docs/FCF_FCP_0048_BTC_PERPETUAL_FUNDING_METHOD_SCHEDULE_EVIDENCE_REGISTRY_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "final_evidence_when_closed": not closed or ((root / "FCF_CURRENT_STATE_FCP_0048_BTC_PERPETUAL_FUNDING_METHOD_SCHEDULE_EVIDENCE_REGISTRY_APP_1_FINAL.md").is_file() and all(finals) and "ALL CHECKS PASSED" in finals[0]),
        ...
        "architecture_registered": all(term in architecture for term in ("## 81. BTC Perpetual Funding Method Schedule Evidence Registry", "signed rate floor and cap", "contract-entry hash")),
        "adr_registered": all(term in adr for term in ("FCF-V2-ADR-048", "Resolve BTC Funding Rules Only From Exact Contract-Bound Evidence", "infer a schedule from observations")),
        "gaps_preserved": all(term in gaps for term in ("V2-FR-GAP-099", "V2-FR-GAP-102", "RESEARCH_REQUIRED")),
        "protocol_registered": "Proposal `FCF-FCP-0048`" in protocol,
        "memory_registered": all(term in memory for term in ("BTC perpetual funding method-schedule registries", "exact FCP-0046 contract", "fail-closed lookup")),
        "delivery_files_exist": all((root / path).is_file() for path in (
            "FCF_CURRENT_STATE_FCP_0048_BTC_PERPETUAL_FUNDING_METHOD_SCHEDULE_EVIDENCE_REGISTRY_APP_1_APPROVED.md",
            "FCF_CURRENT_STATE_FCP_0048_BTC_PERPETUAL_FUNDING_METHOD_SCHEDULE_EVIDENCE_REGISTRY_APP_1_DELIVERED.md",
            "docs/FCF_FCP_0048_BTC_PERPETUAL_FUNDING_METHOD_SCHEDULE_EVIDENCE_REGISTRY_APP_1_D1_D6.md",
            "apps/fcp_0048_btc_perpetual_funding_method_schedule_evidence_registry_app_1/registry.py",
            "tests/fcp_0048_btc_perpetual_funding_method_schedule_evidence_registry_app_1/test_d1_d6.py",
        )),
        "run_all_wired": "control_center_fcp_0048_btc_perpetual_funding_method_schedule_evidence_registry_guard.py" in run_all,
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0048 guard failed: " + ",".join(failed))
