...
DELIVERY_ID = (
    "FCF-FCP-0057-BTC-PERPETUAL-PAPER-STRESS-SCENARIO-COVERAGE-PARAMETER-SCHEMA-GATE-APP-1"
)
AUTHORITIES = tuple(
    Path(value)
    for value in (
...
MARKER = "FCP 0057 BTC PERPETUAL PAPER STRESS SCENARIO COVERAGE PARAMETER SCHEMA GATE APP 1"
...
def build_fcp_0057_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0057"
        ),
        {},
    )
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0057_BTC_PERPETUAL_PAPER_STRESS_SCENARIO_COVERAGE_PARAMETER_SCHEMA_GATE_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0057_BTC_PERPETUAL_PAPER_STRESS_SCENARIO_COVERAGE_PARAMETER_SCHEMA_GATE_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0057_BTC_PERPETUAL_PAPER_STRESS_SCENARIO_COVERAGE_PARAMETER_SCHEMA_GATE_APP_1_FINAL.md",
        "docs/FCF_FCP_0057_BTC_PERPETUAL_PAPER_STRESS_SCENARIO_COVERAGE_PARAMETER_SCHEMA_GATE_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "architecture_registered": all(
            ...
            for term in (
                ...
                "coverage of all eight closed scenario kinds",
                "parameter identifiers, missing parameters, extra parameters",
            )
        ),
        "adr_registered": all(
            term in adr
            for term in (
                "FCF-V2-ADR-057",
                "Require Complete Stress Coverage Before Evaluation",
        ...
        "protocol_registered": "Proposal `FCF-FCP-0057`" in protocol,
        "memory_registered": all(
            term in memory
            for term in (
                "BTC perpetual Paper stress coverage and parameter-schema gates",
                "FCP-0056 lineage",
                "no evaluation or execution authority",
            )
        ),
        "delivery_files_exist": all(
            ...
            for path in (
                ...
                "FCF_CURRENT_STATE_FCP_0057_BTC_PERPETUAL_PAPER_STRESS_SCENARIO_COVERAGE_PARAMETER_SCHEMA_GATE_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0057_BTC_PERPETUAL_PAPER_STRESS_SCENARIO_COVERAGE_PARAMETER_SCHEMA_GATE_APP_1_D1_D6.md",
                "apps/fcp_0057_btc_perpetual_paper_stress_scenario_coverage_parameter_schema_gate_app_1/gate.py",
                "tests/fcp_0057_btc_perpetual_paper_stress_scenario_coverage_parameter_schema_gate_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0057_btc_perpetual_paper_stress_scenario_coverage_parameter_schema_gate_guard.py"
            in run_all
        ),
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0057 guard failed: " + ",".join(failed))
