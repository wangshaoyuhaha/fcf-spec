...
DELIVERY_ID = (
    "FCF-FCP-0036-GUOJIN-QMT-REGISTERED-LOCAL-DAILY-BATCH-COVERAGE-"
...
MARKER = (
    "FCP 0036 GUOJIN QMT REGISTERED LOCAL DAILY BATCH COVERAGE "
    "RECONCILIATION APP 1"
)
...
def build_fcp_0036_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0036"
        ),
        {},
    )
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0036_GUOJIN_QMT_REGISTERED_LOCAL_DAILY_BATCH_COVERAGE_RECONCILIATION_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0036_GUOJIN_QMT_REGISTERED_LOCAL_DAILY_BATCH_COVERAGE_RECONCILIATION_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0036_GUOJIN_QMT_REGISTERED_LOCAL_DAILY_BATCH_COVERAGE_RECONCILIATION_APP_1_FINAL.md",
        "docs/FCF_FCP_0036_GUOJIN_QMT_REGISTERED_LOCAL_DAILY_BATCH_COVERAGE_RECONCILIATION_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "adr_registered": all(
            ...
            for term in (
                "FCF-V2-ADR-036",
                "Require Registered Trading Dates for Batch Completeness",
                "A conflicting overlap",
                "Natural days, weekdays",
            )
        ),
        "gaps_registered": all(
            f"V2-FR-GAP-{index:03d}" in gaps for index in range(107, 110)
        ),
        "protocol_registered": "Proposal `FCF-FCP-0036`" in protocol,
        "memory_registered": all(
            term in memory
            for term in (
                "registered QMT multi-batch coverage reconciliation",
                "conflicting-overlap quarantine",
                "no natural-day",
            )
        ),
        "delivery_files_exist": all(
            ...
            for path in (
                ...
                "FCF_CURRENT_STATE_FCP_0036_GUOJIN_QMT_REGISTERED_LOCAL_DAILY_BATCH_COVERAGE_RECONCILIATION_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0036_GUOJIN_QMT_REGISTERED_LOCAL_DAILY_BATCH_COVERAGE_RECONCILIATION_APP_1_D1_D6.md",
                "apps/fcp_0036_guojin_qmt_registered_local_daily_batch_coverage_reconciliation_app_1/reconciliation.py",
                "tests/fcp_0036_guojin_qmt_registered_local_daily_batch_coverage_reconciliation_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0036_guojin_qmt_registered_local_daily_batch_coverage_reconciliation_guard.py"
            in run_all
        ),
...
def main() -> int:
    report = build_fcp_0036_guard_report()
    if report["ok"] is not True:
        failed = sorted(key for key, value in report["checks"].items() if not value)
        raise SystemExit("FCP-0036 guard failed: " + ",".join(failed))
