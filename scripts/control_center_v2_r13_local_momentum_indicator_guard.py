...
from apps.v2_r13_local_momentum_indicator_foundation_app_1 import V2_R13_MOMENTUM_INDICATOR_BOUNDARY  # noqa: E402

APP_ROOT = Path("apps/v2_r13_local_momentum_indicator_foundation_app_1")
APP_FILES = (
    "__init__.py",
    "acceptance.py",
    "boundary.py",
...
DOCS = tuple(
    Path(f"docs/V2_R13_LOCAL_MOMENTUM_INDICATOR_FOUNDATION_APP_1_D{i}.md")
    for i in range(1, 7)
)
PROHIBITED = (
    "import requests",
...
def build_v2_r13_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
        app_text = "\n".join(
    ...
    boundary = V2_R13_MOMENTUM_INDICATOR_BOUNDARY
    checks = {
        ...
        "deterministic_momentum_present": all(
            token in app_text
            for token in (
                "RSI",
                "RATE_OF_CHANGE",
...
def main() -> int:
    report = build_v2_r13_guard_report()
    if report["ok"] is not True:
        raise SystemExit("V2-R13 local momentum indicator guard failed")
