...
DELIVERY_ID = (
    "FCF-FCP-0045-BTC-CROSS-SOURCE-EXACT-OBSERVATION-DELTA-EVIDENCE-LEDGER-APP-1"
)
AUTHORITIES = tuple(
    Path(value)
    for value in (
...
MARKER = "FCP 0045 BTC CROSS SOURCE EXACT OBSERVATION DELTA EVIDENCE LEDGER APP 1"
...
def build_fcp_0045_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next((item for item in intake.get("proposals", []) if item.get("proposal_id") == "FCF-FCP-0045"), {})
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0045_BTC_CROSS_SOURCE_EXACT_OBSERVATION_DELTA_EVIDENCE_LEDGER_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0045_BTC_CROSS_SOURCE_EXACT_OBSERVATION_DELTA_EVIDENCE_LEDGER_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0045_BTC_CROSS_SOURCE_EXACT_OBSERVATION_DELTA_EVIDENCE_LEDGER_APP_1_FINAL.md",
        "docs/FCF_FCP_0045_BTC_CROSS_SOURCE_EXACT_OBSERVATION_DELTA_EVIDENCE_LEDGER_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "final_evidence_when_closed": not closed or ((root / "FCF_CURRENT_STATE_FCP_0045_BTC_CROSS_SOURCE_EXACT_OBSERVATION_DELTA_EVIDENCE_LEDGER_APP_1_FINAL.md").is_file() and all(finals) and "ALL CHECKS PASSED" in finals[0]),
        ...
        "architecture_registered": all(term in architecture for term in ("## 78. BTC Cross-Source Exact Observation Delta Evidence Ledger", "pairwise union comparison key", "cannot change reconciliation")),
        "adr_registered": all(term in adr for term in ("FCF-V2-ADR-045", "Preserve Exact BTC Observation Deltas Across Every Source Pair", "compare only the all-source")),
        "gaps_preserved": all(term in gaps for term in ("V2-FR-GAP-092", "V2-FR-GAP-095", "RESEARCH_REQUIRED")),
        "protocol_registered": "Proposal `FCF-FCP-0045`" in protocol,
        "memory_registered": all(term in memory for term in ("BTC cross-source exact observation-delta ledgers", "pairwise union key", "immutable upstream reconciliation lineage")),
        "delivery_files_exist": all((root / path).is_file() for path in (
            "FCF_CURRENT_STATE_FCP_0045_BTC_CROSS_SOURCE_EXACT_OBSERVATION_DELTA_EVIDENCE_LEDGER_APP_1_APPROVED.md",
            "FCF_CURRENT_STATE_FCP_0045_BTC_CROSS_SOURCE_EXACT_OBSERVATION_DELTA_EVIDENCE_LEDGER_APP_1_DELIVERED.md",
            "docs/FCF_FCP_0045_BTC_CROSS_SOURCE_EXACT_OBSERVATION_DELTA_EVIDENCE_LEDGER_APP_1_D1_D6.md",
            "apps/fcp_0045_btc_cross_source_exact_observation_delta_evidence_ledger_app_1/ledger.py",
            "tests/fcp_0045_btc_cross_source_exact_observation_delta_evidence_ledger_app_1/test_d1_d6.py",
        )),
        "run_all_wired": "control_center_fcp_0045_btc_cross_source_exact_observation_delta_evidence_ledger_guard.py" in run_all,
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0045 guard failed: " + ",".join(failed))
