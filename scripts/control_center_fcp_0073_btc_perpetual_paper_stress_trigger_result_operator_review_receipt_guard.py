...
DELIVERY_ID = (
    "FCF-FCP-0073-BTC-PERPETUAL-PAPER-STRESS-TRIGGER-RESULT-OPERATOR-REVIEW-RECEIPT-APP-1"
)
MARKER = (
    "FCP 0073 BTC PERPETUAL PAPER STRESS TRIGGER RESULT OPERATOR REVIEW RECEIPT APP 1"
)
AUTHORITIES = tuple(
    Path(value)
    for value in (
...
def build_fcp_0073_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0073"
        ),
        {},
    )
    expected = {
        "FCF_CURRENT_STATE_FCP_0073_BTC_PERPETUAL_PAPER_STRESS_TRIGGER_RESULT_OPERATOR_REVIEW_RECEIPT_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0073_BTC_PERPETUAL_PAPER_STRESS_TRIGGER_RESULT_OPERATOR_REVIEW_RECEIPT_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0073_BTC_PERPETUAL_PAPER_STRESS_TRIGGER_RESULT_OPERATOR_REVIEW_RECEIPT_APP_1_FINAL.md",
        "docs/FCF_FCP_0073_BTC_PERPETUAL_PAPER_STRESS_TRIGGER_RESULT_OPERATOR_REVIEW_RECEIPT_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "architecture_registered": all(
            ...
            for term in (
                ...
                "occurrence only. It does not approve",
                "does not approve or reject evidence",
            )
        ),
        "adr_registered": all(
            term in adr
            for term in (
                "FCF-V2-ADR-073",
                "Record Explicit Review Without Resolution Authority",
        ...
        "protocol_registered": "Proposal `FCF-FCP-0073`" in protocol,
        "memory_registered": "stress trigger-result Operator review receipts" in memory,
        "run_all_wired": (
            "control_center_fcp_0073_btc_perpetual_paper_stress_trigger_result_operator_review_receipt_guard.py"
            in run_all
        ),
