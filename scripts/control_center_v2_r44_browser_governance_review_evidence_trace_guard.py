...
APP_ROOT = Path("apps/v2_r44_browser_governance_review_evidence_trace_presentation_app_1")
...
DOCS = tuple(Path(f"docs/V2_R44_BROWSER_GOVERNANCE_REVIEW_EVIDENCE_TRACE_PRESENTATION_APP_1_D{i}.md") for i in range(1, 7))
PROHIBITED = ("import requests", "import socket", "from urllib", "import subprocess", "websocket", "place_order", "activate_factor")


def build_v2_r44_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    checks = {
        ...
        "trace_contract_present": all(item in text for item in ("observed_field_count", "inferred_field_count", "source_snapshot_hashes", "registered_evidence_only")),
        "browser_integration_present": all(item in browser_text for item in ("review_evidence_trace", "Registered sources", "Source snapshot hashes")),
...
def main() -> int:
    report = build_v2_r44_guard_report()
    if report["ok"] is not True:
        raise SystemExit("V2-R44 browser governance review evidence trace guard failed")
