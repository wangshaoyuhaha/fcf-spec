...
from apps.v2_r35_local_evidence_integrity_foundation_app_1 import V2_R35_LOCAL_EVIDENCE_INTEGRITY_BOUNDARY

APP_ROOT = Path("apps/v2_r35_local_evidence_integrity_foundation_app_1")
APP_FILES = ("__init__.py", "acceptance.py", "boundary.py", "contracts.py", "presentation.py", "registry.py", "resolver.py")
DOCS = tuple(Path(f"docs/V2_R35_LOCAL_EVIDENCE_INTEGRITY_FOUNDATION_APP_1_D{i}.md") for i in range(1, 7))
PROHIBITED = ("import requests", "import socket", "from urllib", "import subprocess", "websocket", "place_order", "score_candidate")


def build_v2_r35_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    checks = {
        ...
        "contract_present": all(item in text for item in ("RegisteredEvidenceArtifact", "EvidenceFreshnessPolicy", "RESOLVED_OBSERVED", "RESOLVED_INFERRED", "DIGEST_VERIFIED", "REVISION_LINEAGE_VERIFIED")),
        "no_prohibited_runtime": all(item not in text.lower() for item in PROHIBITED),
        "boundary_closed": boundary.local_only and boundary.registered_artifact_only and not boundary.network_access_allowed and not boundary.live_source_allowed and not boundary.inferred_as_observed_allowed and not boundary.digest_bypass_allowed and not boundary.broken_revision_lineage_allowed and not boundary.stale_as_fresh_allowed and not boundary.order_or_execution_allowed,
    }
    return {"checks": checks, "ok": all(checks.values())}


def main() -> int:
    report = build_v2_r35_guard_report()
    if report["ok"] is not True:
        raise SystemExit("V2-R35 evidence integrity guard failed")
