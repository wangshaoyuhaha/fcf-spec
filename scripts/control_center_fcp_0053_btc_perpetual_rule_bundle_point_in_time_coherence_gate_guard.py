...
DELIVERY_ID = (
    "FCF-FCP-0053-BTC-PERPETUAL-RULE-BUNDLE-POINT-IN-TIME-COHERENCE-GATE-APP-1"
)
AUTHORITIES = tuple(
    Path(value)
    for value in (
...
MARKER = "FCP 0053 BTC PERPETUAL RULE BUNDLE POINT IN TIME COHERENCE GATE APP 1"
...
def build_fcp_0053_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0053"
        ),
        {},
    )
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0053_BTC_PERPETUAL_RULE_BUNDLE_POINT_IN_TIME_COHERENCE_GATE_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0053_BTC_PERPETUAL_RULE_BUNDLE_POINT_IN_TIME_COHERENCE_GATE_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0053_BTC_PERPETUAL_RULE_BUNDLE_POINT_IN_TIME_COHERENCE_GATE_APP_1_FINAL.md",
        "docs/FCF_FCP_0053_BTC_PERPETUAL_RULE_BUNDLE_POINT_IN_TIME_COHERENCE_GATE_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "architecture_registered": all(
            ...
            for term in (
                ...
                "same exact FCP-0046",
                "cross-contract, cross-registry, and effective-time-incoherent",
            )
        ),
        "adr_registered": all(
            term in adr
            for term in (
                "FCF-V2-ADR-053",
                "Resolve BTC Perpetual Rule Bundles Only from Coherent Point-in-Time Lineage",
                "combine independent registry lookups",
            )
        ),
        "gaps_preserved": all(
            term in gaps
        ...
        "protocol_registered": "Proposal `FCF-FCP-0053`" in protocol,
        "memory_registered": all(
            term in memory
            for term in (
                "BTC perpetual point-in-time rule-bundle coherence gates",
                "exact FCP-0046",
                "no account-dependent calculation or execution authority",
            )
        ),
        "delivery_files_exist": all(
            ...
            for path in (
                ...
                "FCF_CURRENT_STATE_FCP_0053_BTC_PERPETUAL_RULE_BUNDLE_POINT_IN_TIME_COHERENCE_GATE_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0053_BTC_PERPETUAL_RULE_BUNDLE_POINT_IN_TIME_COHERENCE_GATE_APP_1_D1_D6.md",
                "apps/fcp_0053_btc_perpetual_rule_bundle_point_in_time_coherence_gate_app_1/resolver.py",
                "tests/fcp_0053_btc_perpetual_rule_bundle_point_in_time_coherence_gate_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0053_btc_perpetual_rule_bundle_point_in_time_coherence_gate_guard.py"
            in run_all
        ),
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0053 guard failed: " + ",".join(failed))
