...
DELIVERY_ID = "FCF-FCP-0066-BTC-PERPETUAL-PAPER-STRESS-EVALUATION-DIRECTION-SEMANTICS-REGISTRY-APP-1"
MARKER = "FCP 0066 BTC PERPETUAL PAPER STRESS EVALUATION DIRECTION SEMANTICS REGISTRY APP 1"
...
def build_fcp_0066_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next((item for item in intake.get("proposals", []) if item.get("proposal_id") == "FCF-FCP-0066"), {})
    expected = {
        "FCF_CURRENT_STATE_FCP_0066_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_DIRECTION_SEMANTICS_REGISTRY_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0066_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_DIRECTION_SEMANTICS_REGISTRY_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0066_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_DIRECTION_SEMANTICS_REGISTRY_APP_1_FINAL.md",
        "docs/FCF_FCP_0066_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_DIRECTION_SEMANTICS_REGISTRY_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "final_evidence_when_closed": not closed or ((root / "FCF_CURRENT_STATE_FCP_0066_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_DIRECTION_SEMANTICS_REGISTRY_APP_1_FINAL.md").is_file() and all(finals) and "ALL CHECKS PASSED" in finals[0]),
        ...
        "architecture_registered": all(term in architecture for term in ("## 99. BTC Perpetual Paper Stress Evaluation Direction Semantics Registry", "absolute baseline divergence", "equality policy")),
        "adr_registered": all(term in adr for term in ("FCF-V2-ADR-066", "Register Explicit Stress Direction Before Formulas", "inclusive boundary")),
        "gaps_preserved": all(term in gaps for term in ("V2-FR-GAP-098", "V2-FR-GAP-099", "V2-FR-GAP-100", "V2-FR-GAP-101", "RESEARCH_REQUIRED")),
        "protocol_registered": "Proposal `FCF-FCP-0066`" in protocol,
        "memory_registered": all(term in memory for term in ("direction-semantics registries", "exact typed FCP-0065 lineage", "no formula or evaluation authority")),
        "delivery_files_exist": all((root / path).is_file() for path in (
            "FCF_CURRENT_STATE_FCP_0066_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_DIRECTION_SEMANTICS_REGISTRY_APP_1_APPROVED.md",
            "FCF_CURRENT_STATE_FCP_0066_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_DIRECTION_SEMANTICS_REGISTRY_APP_1_DELIVERED.md",
            "docs/FCF_FCP_0066_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_DIRECTION_SEMANTICS_REGISTRY_APP_1_D1_D6.md",
            "apps/fcp_0066_btc_perpetual_paper_stress_evaluation_direction_semantics_registry_app_1/registry.py",
            "tests/fcp_0066_btc_perpetual_paper_stress_evaluation_direction_semantics_registry_app_1/test_d1_d6.py",
        )),
        "run_all_wired": "control_center_fcp_0066_btc_perpetual_paper_stress_evaluation_direction_semantics_registry_guard.py" in run_all,
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0066 guard failed: " + failures)
