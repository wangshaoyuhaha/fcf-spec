...
from apps.v2_r14_local_trend_indicator_foundation_app_1 import V2_R14_TREND_INDICATOR_BOUNDARY  # noqa: E402

APP_ROOT = Path("apps/v2_r14_local_trend_indicator_foundation_app_1")
APP_FILES = (
    "__init__.py",
    "acceptance.py",
    "boundary.py",
...
DOCS = tuple(
    Path(f"docs/V2_R14_LOCAL_TREND_INDICATOR_FOUNDATION_APP_1_D{i}.md")
    for i in range(1, 7)
)
PROHIBITED = (
    "import requests",
...
def build_v2_r14_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
        app_text = "\n".join(
    ...
    boundary = V2_R14_TREND_INDICATOR_BOUNDARY
    checks = {
        ...
        "deterministic_trend_present": all(
            token in app_text
            for token in (
                "EMA",
                "MACD",
...
def main() -> int:
    report = build_v2_r14_guard_report()
    if report["ok"] is not True:
        raise SystemExit("V2-R14 local trend indicator guard failed")
