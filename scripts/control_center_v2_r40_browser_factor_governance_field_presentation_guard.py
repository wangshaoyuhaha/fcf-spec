...
APP_ROOT = Path(
    "apps/v2_r40_browser_factor_governance_field_presentation_app_1"
)
APP_FILES = (
    "__init__.py",
    "acceptance.py",
...
DOCS = tuple(
    Path(f"docs/V2_R40_BROWSER_FACTOR_GOVERNANCE_FIELD_PRESENTATION_APP_1_D{i}.md")
    for i in range(1, 7)
)
PROHIBITED = (
    "import requests",
...
def build_v2_r40_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
        text = "\n".join(
    ...
    checks = {
        ...
        "explicit_field_presentation_present": all(
            item in text
            for item in (
                "OBSERVED",
                "INFERRED",
...
def main() -> int:
    report = build_v2_r40_guard_report()
    if report["ok"] is not True:
        raise SystemExit("V2-R40 browser governance field presentation guard failed")
