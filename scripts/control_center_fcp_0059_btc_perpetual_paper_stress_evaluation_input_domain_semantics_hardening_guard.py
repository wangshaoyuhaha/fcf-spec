...
DELIVERY_ID = (
    "FCF-FCP-0059-BTC-PERPETUAL-PAPER-STRESS-EVALUATION-INPUT-DOMAIN-SEMANTICS-HARDENING-APP-1"
)
AUTHORITIES = tuple(
    Path(value)
    for value in (
...
MARKER = (
    "FCP 0059 BTC PERPETUAL PAPER STRESS EVALUATION INPUT DOMAIN SEMANTICS HARDENING APP 1"
)
...
def build_fcp_0059_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0059"
        ),
        {},
    )
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0059_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_INPUT_DOMAIN_SEMANTICS_HARDENING_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0059_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_INPUT_DOMAIN_SEMANTICS_HARDENING_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0059_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_INPUT_DOMAIN_SEMANTICS_HARDENING_APP_1_FINAL.md",
        "docs/FCF_FCP_0059_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_INPUT_DOMAIN_SEMANTICS_HARDENING_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "architecture_registered": all(
            ...
            for term in (
                ...
                "signed finite funding rates",
                "bounded liquidation-distance ratios",
            )
        ),
        "adr_registered": all(
            term in adr
            for term in (
                "FCF-V2-ADR-059",
                "Use Metric-Specific Domains For Stress Inputs",
                "Funding-reference rates are finite",
            )
        ),
        "gaps_preserved": all(
            term in gaps
        ...
        "protocol_registered": "Proposal `FCF-FCP-0059`" in protocol,
        "memory_registered": all(
            term in memory
            for term in (
                "stress-input domain hardening",
                "finite signed funding rates",
                "immutable validation hashes",
            )
        ),
        "delivery_files_exist": all(
            ...
            for path in (
                ...
                "FCF_CURRENT_STATE_FCP_0059_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_INPUT_DOMAIN_SEMANTICS_HARDENING_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0059_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_INPUT_DOMAIN_SEMANTICS_HARDENING_APP_1_D1_D6.md",
                "apps/fcp_0059_btc_perpetual_paper_stress_evaluation_input_domain_semantics_hardening_app_1/hardening.py",
                "tests/fcp_0059_btc_perpetual_paper_stress_evaluation_input_domain_semantics_hardening_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0059_btc_perpetual_paper_stress_evaluation_input_domain_semantics_hardening_guard.py"
            in run_all
        ),
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0059 guard failed: " + ",".join(failed))
