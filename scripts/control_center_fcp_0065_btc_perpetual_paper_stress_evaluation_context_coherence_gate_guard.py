...
DELIVERY_ID = "FCF-FCP-0065-BTC-PERPETUAL-PAPER-STRESS-EVALUATION-CONTEXT-COHERENCE-GATE-APP-1"
MARKER = "FCP 0065 BTC PERPETUAL PAPER STRESS EVALUATION CONTEXT COHERENCE GATE APP 1"
...
def build_fcp_0065_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next((item for item in intake.get("proposals", []) if item.get("proposal_id") == "FCF-FCP-0065"), {})
    expected = {
        "FCF_CURRENT_STATE_FCP_0065_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_CONTEXT_COHERENCE_GATE_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0065_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_CONTEXT_COHERENCE_GATE_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0065_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_CONTEXT_COHERENCE_GATE_APP_1_FINAL.md",
        "docs/FCF_FCP_0065_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_CONTEXT_COHERENCE_GATE_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "final_evidence_when_closed": not closed or ((root / "FCF_CURRENT_STATE_FCP_0065_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_CONTEXT_COHERENCE_GATE_APP_1_FINAL.md").is_file() and all(finals) and "ALL CHECKS PASSED" in finals[0]),
        ...
        "architecture_registered": all(term in architecture for term in ("## 98. BTC Perpetual Paper Stress Evaluation Context Coherence Gate", "exact typed FCP-0056", "monotonic UTC lineage")),
        "adr_registered": all(term in adr for term in ("FCF-V2-ADR-065", "Bind Complete Typed Context Before Stress Formulas", "orphan operands")),
        "gaps_preserved": all(term in gaps for term in ("V2-FR-GAP-098", "V2-FR-GAP-099", "V2-FR-GAP-100", "V2-FR-GAP-101", "RESEARCH_REQUIRED")),
        "protocol_registered": "Proposal `FCF-FCP-0065`" in protocol,
        "memory_registered": all(term in memory for term in ("stress-evaluation context coherence gates", "FCP-0056, FCP-0062, and FCP-0064 lineage", "no direction or evaluation authority")),
        "delivery_files_exist": all((root / path).is_file() for path in (
            "FCF_CURRENT_STATE_FCP_0065_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_CONTEXT_COHERENCE_GATE_APP_1_APPROVED.md",
            "FCF_CURRENT_STATE_FCP_0065_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_CONTEXT_COHERENCE_GATE_APP_1_DELIVERED.md",
            "docs/FCF_FCP_0065_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_CONTEXT_COHERENCE_GATE_APP_1_D1_D6.md",
            "apps/fcp_0065_btc_perpetual_paper_stress_evaluation_context_coherence_gate_app_1/gate.py",
            "tests/fcp_0065_btc_perpetual_paper_stress_evaluation_context_coherence_gate_app_1/test_d1_d6.py",
        )),
        "run_all_wired": "control_center_fcp_0065_btc_perpetual_paper_stress_evaluation_context_coherence_gate_guard.py" in run_all,
...
def main() -> int:
    report = build_fcp_0065_guard_report()
    if report["ok"] is not True:
        failures = ",".join(sorted(key for key, value in report["checks"].items() if not value))
        raise SystemExit("FCP-0065 guard failed: " + failures)
