...
DELIVERY_ID = (
    "FCF-FCP-0058-BTC-PERPETUAL-PAPER-STRESS-EVALUATION-INPUT-EVIDENCE-REGISTRY-APP-1"
)
AUTHORITIES = tuple(
    Path(value)
    for value in (
...
MARKER = (
    "FCP 0058 BTC PERPETUAL PAPER STRESS EVALUATION INPUT EVIDENCE REGISTRY APP 1"
)
...
def build_fcp_0058_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0058"
        ),
        {},
    )
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0058_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_INPUT_EVIDENCE_REGISTRY_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0058_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_INPUT_EVIDENCE_REGISTRY_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0058_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_INPUT_EVIDENCE_REGISTRY_APP_1_FINAL.md",
        "docs/FCF_FCP_0058_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_INPUT_EVIDENCE_REGISTRY_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "architecture_registered": all(
            ...
            for term in (
                ...
                "one evaluation-input observation for every closed scenario kind",
                "event time, availability time",
            )
        ),
        "adr_registered": all(
            term in adr
            for term in (
                "FCF-V2-ADR-058",
                "Require Registered Stress Inputs Before Evaluation",
                "source artifact, digest, rights",
            )
        ),
        "gaps_preserved": all(
            term in gaps
        ...
        "protocol_registered": "Proposal `FCF-FCP-0058`" in protocol,
        "memory_registered": all(
            term in memory
            for term in (
                "stress-evaluation input evidence registries",
                "FCP-0057 lineage",
        ...
        "delivery_files_exist": all(
            ...
            for path in (
                ...
                "FCF_CURRENT_STATE_FCP_0058_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_INPUT_EVIDENCE_REGISTRY_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0058_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_INPUT_EVIDENCE_REGISTRY_APP_1_D1_D6.md",
                "apps/fcp_0058_btc_perpetual_paper_stress_evaluation_input_evidence_registry_app_1/registry.py",
                "tests/fcp_0058_btc_perpetual_paper_stress_evaluation_input_evidence_registry_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0058_btc_perpetual_paper_stress_evaluation_input_evidence_registry_guard.py"
            in run_all
        ),
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0058 guard failed: " + ",".join(failed))
