...
DELIVERY_ID = "FCF-FCP-0074-BTC-PERPETUAL-PAPER-SIGNAL-EVIDENCE-SEPARATION-CONTRACT-APP-1"
MARKER = "FCP 0074 BTC PERPETUAL PAPER SIGNAL EVIDENCE SEPARATION CONTRACT APP 1"
AUTHORITIES = tuple(
    Path(value)
    for value in (
...
def build_fcp_0074_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next((item for item in intake.get("proposals", []) if item.get("proposal_id") == "FCF-FCP-0074"), {})
    expected = {
        "FCF_CURRENT_STATE_FCP_0074_BTC_PERPETUAL_PAPER_SIGNAL_EVIDENCE_SEPARATION_CONTRACT_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0074_BTC_PERPETUAL_PAPER_SIGNAL_EVIDENCE_SEPARATION_CONTRACT_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0074_BTC_PERPETUAL_PAPER_SIGNAL_EVIDENCE_SEPARATION_CONTRACT_APP_1_FINAL.md",
        "docs/FCF_FCP_0074_BTC_PERPETUAL_PAPER_SIGNAL_EVIDENCE_SEPARATION_CONTRACT_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "architecture_registered": all(term in architecture for term in ("## 107. BTC Perpetual Paper Signal Evidence Separation Contract", "REUSABLE_MARKET_SIGNAL", "does not close GAP-103")),
        "adr_registered": all(term in adr for term in ("FCF-V2-ADR-074", "Separate Reusable Signal Evidence From Derivative Mechanics", "claim profitability")),
        "gap_preserved": "V2-FR-GAP-103" in gaps and "| NOT_IMPLEMENTED |" in next(line for line in gaps.splitlines() if "V2-FR-GAP-103" in line),
        "protocol_registered": "Proposal `FCF-FCP-0074`" in protocol,
        "memory_registered": "signal-evidence separation contracts" in memory,
        "run_all_wired": "control_center_fcp_0074_btc_perpetual_paper_signal_evidence_separation_contract_guard.py" in run_all,
        "delivery_files_exist": all((root / path).is_file() for path in expected if "FINAL" not in path) and ((root / next(path for path in expected if "FINAL" in path)).is_file() if closed else True),
