...
DELIVERY_ID = "FCF-FCP-0068-BTC-PERPETUAL-PAPER-STRESS-EVALUATION-TRIGGER-PREDICATE-SEMANTICS-REGISTRY-APP-1"
MARKER = "FCP 0068 BTC PERPETUAL PAPER STRESS EVALUATION TRIGGER PREDICATE SEMANTICS REGISTRY APP 1"
...
def build_fcp_0068_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next((item for item in intake.get("proposals", []) if item.get("proposal_id") == "FCF-FCP-0068"), {})
    expected = {
        "FCF_CURRENT_STATE_FCP_0068_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_TRIGGER_PREDICATE_SEMANTICS_REGISTRY_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0068_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_TRIGGER_PREDICATE_SEMANTICS_REGISTRY_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0068_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_TRIGGER_PREDICATE_SEMANTICS_REGISTRY_APP_1_FINAL.md",
        "docs/FCF_FCP_0068_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_TRIGGER_PREDICATE_SEMANTICS_REGISTRY_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "final_evidence_when_closed": not closed or ((root / "FCF_CURRENT_STATE_FCP_0068_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_TRIGGER_PREDICATE_SEMANTICS_REGISTRY_APP_1_FINAL.md").is_file() and all(finals) and "ALL CHECKS PASSED" in finals[0]),
        ...
        "architecture_registered": all(term in architecture for term in ("## 101. BTC Perpetual Paper Stress Evaluation Trigger Predicate Semantics Registry", "strict measure-greater-than-parameter", "inclusive measure-less-than-or-equal-to-parameter")),
        "adr_registered": all(term in adr for term in ("FCF-V2-ADR-068", "Bind Closed Stress Trigger Predicates Before Evaluation", "strictness")),
        "gaps_preserved": all(term in gaps for term in ("V2-FR-GAP-098", "V2-FR-GAP-099", "V2-FR-GAP-100", "V2-FR-GAP-101", "RESEARCH_REQUIRED")),
        "protocol_registered": "Proposal `FCF-FCP-0068`" in protocol,
        "memory_registered": all(term in memory for term in ("trigger-predicate semantics registries", "exact typed FCP-0067 lineage", "evaluation or calculation authority")),
        "delivery_files_exist": all((root / path).is_file() for path in (
            "FCF_CURRENT_STATE_FCP_0068_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_TRIGGER_PREDICATE_SEMANTICS_REGISTRY_APP_1_APPROVED.md",
            "FCF_CURRENT_STATE_FCP_0068_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_TRIGGER_PREDICATE_SEMANTICS_REGISTRY_APP_1_DELIVERED.md",
            "docs/FCF_FCP_0068_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_TRIGGER_PREDICATE_SEMANTICS_REGISTRY_APP_1_D1_D6.md",
            "apps/fcp_0068_btc_perpetual_paper_stress_evaluation_trigger_predicate_semantics_registry_app_1/registry.py",
            "tests/fcp_0068_btc_perpetual_paper_stress_evaluation_trigger_predicate_semantics_registry_app_1/test_d1_d6.py",
        )),
        "run_all_wired": "control_center_fcp_0068_btc_perpetual_paper_stress_evaluation_trigger_predicate_semantics_registry_guard.py" in run_all,
...
def main() -> int:
    report = build_fcp_0068_guard_report()
    if report["ok"] is not True:
        failures = ",".join(sorted(key for key, value in report["checks"].items() if not value))
        raise SystemExit("FCP-0068 guard failed: " + failures)
