...
from apps.v2_r19_local_percentage_price_oscillator_foundation_app_1 import (  # noqa: E402
    V2_R19_PERCENTAGE_OSCILLATOR_BOUNDARY,
)

APP_ROOT = Path("apps/v2_r19_local_percentage_price_oscillator_foundation_app_1")
APP_FILES = (
    "__init__.py",
    "acceptance.py",
    "boundary.py",
...
DOCS = tuple(
    Path(f"docs/V2_R19_LOCAL_PERCENTAGE_PRICE_OSCILLATOR_FOUNDATION_APP_1_D{i}.md")
    for i in range(1, 7)
)
PROHIBITED = (
    "import requests",
...
def build_v2_r19_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
        app_text = "\n".join(
    ...
    boundary = V2_R19_PERCENTAGE_OSCILLATOR_BOUNDARY
    checks = {
        ...
        "deterministic_ppo_present": all(
            token in app_text
            for token in (
                "PPO",
                "ROUND_HALF_EVEN",
                "INSUFFICIENT_WINDOW_BLOCKED",
                "PERCENTAGE_OSCILLATOR_READY",
                "ppo_line",
                "signal_line",
            )
        ),
        ...
        and not boundary.direction_label_allowed
...
def main() -> int:
    ...
    if report["ok"] is not True:
        raise SystemExit("V2-R19 local percentage price oscillator guard failed")
