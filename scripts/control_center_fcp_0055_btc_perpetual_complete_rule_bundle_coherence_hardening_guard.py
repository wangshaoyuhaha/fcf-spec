...
DELIVERY_ID = (
    "FCF-FCP-0055-BTC-PERPETUAL-COMPLETE-RULE-BUNDLE-COHERENCE-HARDENING-APP-1"
)
AUTHORITIES = tuple(
    Path(value)
    for value in (
...
MARKER = "FCP 0055 BTC PERPETUAL COMPLETE RULE BUNDLE COHERENCE HARDENING APP 1"
...
def build_fcp_0055_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0055"
        ),
        {},
    )
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0055_BTC_PERPETUAL_COMPLETE_RULE_BUNDLE_COHERENCE_HARDENING_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0055_BTC_PERPETUAL_COMPLETE_RULE_BUNDLE_COHERENCE_HARDENING_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0055_BTC_PERPETUAL_COMPLETE_RULE_BUNDLE_COHERENCE_HARDENING_APP_1_FINAL.md",
        "docs/FCF_FCP_0055_BTC_PERPETUAL_COMPLETE_RULE_BUNDLE_COHERENCE_HARDENING_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "architecture_registered": all(
            ...
            for term in (
                ...
                "same exact FCP-0046 contract-registry hash",
                "effective-time-incoherent evidence fails closed",
            )
        ),
        "adr_registered": all(
            term in adr
            for term in (
                "FCF-V2-ADR-055",
                "Require Liquidation Evidence In The Complete BTC Rule Bundle",
                "attach an FCP-0054 hash without resolving",
            )
        ),
        "gaps_preserved": all(
            term in gaps
        ...
        "protocol_registered": "Proposal `FCF-FCP-0055`" in protocol,
        "memory_registered": all(
            term in memory
            for term in (
                "BTC perpetual complete-rule-bundle coherence hardening",
                "exact FCP-0053",
                "later Paper stress or risk-gate consumption",
            )
        ),
        "delivery_files_exist": all(
            ...
            for path in (
                ...
                "FCF_CURRENT_STATE_FCP_0055_BTC_PERPETUAL_COMPLETE_RULE_BUNDLE_COHERENCE_HARDENING_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0055_BTC_PERPETUAL_COMPLETE_RULE_BUNDLE_COHERENCE_HARDENING_APP_1_D1_D6.md",
                "apps/fcp_0055_btc_perpetual_complete_rule_bundle_coherence_hardening_app_1/resolver.py",
                "tests/fcp_0055_btc_perpetual_complete_rule_bundle_coherence_hardening_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0055_btc_perpetual_complete_rule_bundle_coherence_hardening_guard.py"
            in run_all
        ),
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0055 guard failed: " + ",".join(failed))
