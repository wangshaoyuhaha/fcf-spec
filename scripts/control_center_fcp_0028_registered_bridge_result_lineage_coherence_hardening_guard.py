...
DELIVERY_ID = (
    "FCF-FCP-0028-REGISTERED-BRIDGE-RESULT-LINEAGE-COHERENCE-HARDENING-APP-1"
)
AUTHORITIES = (
...
APPROVAL_START = "<!-- FCP 0028 REGISTERED BRIDGE RESULT LINEAGE COHERENCE HARDENING APP 1 APPROVAL START -->"
APPROVAL_END = "<!-- FCP 0028 REGISTERED BRIDGE RESULT LINEAGE COHERENCE HARDENING APP 1 APPROVAL END -->"
LOCK_START = "<!-- FCP 0028 REGISTERED BRIDGE RESULT LINEAGE COHERENCE HARDENING APP 1 LOCK START -->"
LOCK_END = "<!-- FCP 0028 REGISTERED BRIDGE RESULT LINEAGE COHERENCE HARDENING APP 1 LOCK END -->"
FINAL_START = "<!-- FCP 0028 REGISTERED BRIDGE RESULT LINEAGE COHERENCE HARDENING APP 1 FINAL START -->"
FINAL_END = "<!-- FCP 0028 REGISTERED BRIDGE RESULT LINEAGE COHERENCE HARDENING APP 1 FINAL END -->"
...
def build_fcp_0028_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
        ...
        btc_bridge = (
            root
            / "apps/fcp_0022_btc_local_export_canonicalization_bridge_app_1/bridge.py"
        ).read_text(encoding="ascii")
        run_all = (root / "scripts/run_all_checks.py").read_text(encoding="ascii")
        readable = True
    except (FileNotFoundError, UnicodeDecodeError, json.JSONDecodeError):
        texts, manifest, intake = (), {}, {}
        a_bridge = btc_contracts = btc_bridge = run_all = ""
        readable = False
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0028"
        ),
        {},
    )
    checks = {
        ...
        "a_share_coherence": all(
            marker in a_bridge
            for marker in (
                "canonical registration digest disagrees with bytes",
                "canonical registration byte length disagrees with bytes",
                "bridge manifest digest disagrees with canonical bytes",
                "bridge manifest row count disagrees with canonical bytes",
            )
        ),
        "btc_coherence": all(
            marker in btc_contracts
            for marker in (
                "BTC_OBSERVATION_TYPES",
                "canonical_observations_ndjson",
                "bridge manifest canonical artifact identity disagrees",
                "bridge manifest observation hashes disagree",
                "observation artifact lineage disagrees with registration",
                "canonical bytes disagree with typed observations",
            )
        )
        and "canonical = canonical_observations_ndjson(observations)" in btc_bridge,
        "delivery_files_exist": all(
            (root / path).is_file()
            for path in (
                "FCF_CURRENT_STATE_FCP_0028_REGISTERED_BRIDGE_RESULT_LINEAGE_COHERENCE_HARDENING_APP_1_APPROVED.md",
                "FCF_CURRENT_STATE_FCP_0028_REGISTERED_BRIDGE_RESULT_LINEAGE_COHERENCE_HARDENING_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0028_REGISTERED_BRIDGE_RESULT_LINEAGE_COHERENCE_HARDENING_APP_1_D1_D6.md",
                "tests/fcp_0028_registered_bridge_result_lineage_coherence_hardening_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0028_registered_bridge_result_lineage_coherence_hardening_guard.py"
            in run_all
        ),
...
def main() -> int:
    report = build_fcp_0028_guard_report()
    if report["ok"] is not True:
        failed = ",".join(
            sorted(name for name, value in report["checks"].items() if not value)
        )
        raise SystemExit("FCP-0028 guard failed: " + failed)
