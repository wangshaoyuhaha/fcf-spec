...
DELIVERY_ID = "FCF-FCP-0023-BTC-CROSS-SOURCE-VENUE-QUALITY-RECONCILIATION-APP-1"
AUTHORITIES = (
...
APPROVAL_START = "<!-- FCP 0023 BTC CROSS SOURCE VENUE QUALITY RECONCILIATION APP 1 APPROVAL START -->"
APPROVAL_END = "<!-- FCP 0023 BTC CROSS SOURCE VENUE QUALITY RECONCILIATION APP 1 APPROVAL END -->"
LOCK_START = "<!-- FCP 0023 BTC CROSS SOURCE VENUE QUALITY RECONCILIATION APP 1 LOCK START -->"
LOCK_END = "<!-- FCP 0023 BTC CROSS SOURCE VENUE QUALITY RECONCILIATION APP 1 LOCK END -->"
FINAL_START = "<!-- FCP 0023 BTC CROSS SOURCE VENUE QUALITY RECONCILIATION APP 1 FINAL START -->"
FINAL_END = "<!-- FCP 0023 BTC CROSS SOURCE VENUE QUALITY RECONCILIATION APP 1 FINAL END -->"
...
def build_fcp_0023_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
        ...
        reconciliation = (root / "apps/fcp_0023_btc_cross_source_venue_quality_reconciliation_app_1/reconciliation.py").read_text(encoding="ascii")
        readable = True
    ...
    proposal = next((item for item in intake.get("proposals", []) if item.get("proposal_id") == "FCF-FCP-0023"), {})
    checks = {
        ...
        "reconciliation_fail_closed": all(term in reconciliation for term in ("reconciliation requires at least two typed BTC datasets", "COVERAGE_GAP", "VENUE_MISMATCH", "CLOCK_MISMATCH", "SEQUENCE_MISMATCH", "BOOK_MISMATCH", "MARK_PRICE_MISMATCH", "FUNDING_RATE_MISMATCH", "QUARANTINE_REVIEW_REQUIRED")),
        "delivery_files_exist": all((root / path).is_file() for path in (
            "FCF_CURRENT_STATE_FCP_0023_BTC_CROSS_SOURCE_VENUE_QUALITY_RECONCILIATION_APP_1_APPROVED.md",
            "FCF_CURRENT_STATE_FCP_0023_BTC_CROSS_SOURCE_VENUE_QUALITY_RECONCILIATION_APP_1_DELIVERED.md",
            "docs/FCF_FCP_0023_BTC_CROSS_SOURCE_VENUE_QUALITY_RECONCILIATION_APP_1_D1_D6.md",
            "tests/fcp_0023_btc_cross_source_venue_quality_reconciliation_app_1/test_d1_d6.py",
        )),
        "run_all_wired": "control_center_fcp_0023_btc_cross_source_venue_quality_reconciliation_guard.py" in (root / "scripts/run_all_checks.py").read_text(encoding="ascii"),
        "no_provider_runtime": all(term not in (contracts + reconciliation).lower() for term in ("import requests", "import socket", "import websocket", "import ccxt", "api_key", "secret_key", "password")),
...
def main() -> int:
    ...
    if not report["ok"]:
        ...
        raise SystemExit("FCP-0023 guard failed: " + ",".join(failed))
