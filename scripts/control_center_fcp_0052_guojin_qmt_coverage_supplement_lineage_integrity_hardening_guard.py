...
MARKER = "FCP 0052 GUOJIN QMT COVERAGE SUPPLEMENT LINEAGE INTEGRITY HARDENING APP 1"
...
def build_fcp_0052_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
        ...
        intake = json.loads(
            (root / "FCF_FUTURE_CAPABILITY_INTAKE_REGISTER.json").read_text(encoding="ascii")
        )
        architecture = (
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0052"
        ),
        {},
    )
    checks = {
        ...
        or (
            (root / "FCF_CURRENT_STATE_FCP_0052_GUOJIN_QMT_COVERAGE_SUPPLEMENT_LINEAGE_INTEGRITY_HARDENING_APP_1_FINAL.md").is_file()
            and all(finals)
        ...
        "architecture_registered": all(
            ...
            for term in (
                ...
                "Cross-instrument",
                "derive FCP-0051 supplement hashes",
            )
        ),
        "adr_registered": all(
            term in adr
            for term in (
                "FCF-V2-ADR-052",
                "Typed Coherent Lineage",
                "accept arbitrary hashes or counts",
            )
        ),
        "gaps_preserved": all(
        ...
        "protocol_registered": "Proposal `FCF-FCP-0052`" in protocol,
        "memory_registered": all(
            term in memory
            for term in (
                "QMT coverage-supplement lineage hardening",
                "typed FCP-0037 and FCP-0036",
                "no arbitrary digest proof",
            )
        ),
        "delivery_files_exist": all(
            (root / path).is_file()
            for path in (
                "FCF_CURRENT_STATE_FCP_0052_GUOJIN_QMT_COVERAGE_SUPPLEMENT_LINEAGE_INTEGRITY_HARDENING_APP_1_APPROVED.md",
                "FCF_CURRENT_STATE_FCP_0052_GUOJIN_QMT_COVERAGE_SUPPLEMENT_LINEAGE_INTEGRITY_HARDENING_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0052_GUOJIN_QMT_COVERAGE_SUPPLEMENT_LINEAGE_INTEGRITY_HARDENING_APP_1_D1_D6.md",
                "apps/fcp_0052_guojin_qmt_coverage_supplement_lineage_integrity_hardening_app_1/hardening.py",
                "tests/fcp_0052_guojin_qmt_coverage_supplement_lineage_integrity_hardening_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0052_guojin_qmt_coverage_supplement_lineage_integrity_hardening_guard.py"
            in run_all
        ),
...
def main() -> int:
    report = build_fcp_0052_guard_report()
    if report["ok"] is not True:
        failed = sorted(key for key, value in report["checks"].items() if not value)
        raise SystemExit("FCP-0052 guard failed: " + ",".join(failed))
