...
DELIVERY_ID = (
    "FCF-FCP-0038-A-SHARE-REGISTERED-SAME-CALENDAR-CROSS-SOURCE-COVERAGE-"
...
MARKER = (
    "FCP 0038 A SHARE REGISTERED SAME CALENDAR CROSS SOURCE COVERAGE "
    "RECONCILIATION APP 1"
)
...
def build_fcp_0038_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next((item for item in intake.get("proposals", []) if item.get("proposal_id") == "FCF-FCP-0038"), {})
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0038_A_SHARE_REGISTERED_SAME_CALENDAR_CROSS_SOURCE_COVERAGE_RECONCILIATION_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0038_A_SHARE_REGISTERED_SAME_CALENDAR_CROSS_SOURCE_COVERAGE_RECONCILIATION_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0038_A_SHARE_REGISTERED_SAME_CALENDAR_CROSS_SOURCE_COVERAGE_RECONCILIATION_APP_1_FINAL.md",
        "docs/FCF_FCP_0038_A_SHARE_REGISTERED_SAME_CALENDAR_CROSS_SOURCE_COVERAGE_RECONCILIATION_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "final_evidence_when_closed": not closed or ((root / "FCF_CURRENT_STATE_FCP_0038_A_SHARE_REGISTERED_SAME_CALENDAR_CROSS_SOURCE_COVERAGE_RECONCILIATION_APP_1_FINAL.md").is_file() and all(finals) and "ALL CHECKS PASSED" in finals[0]),
        ...
        "architecture_registered": all(term in architecture for term in ("## 71. A-Share Same-Calendar Cross-Source Coverage Reconciliation", "Union coverage cannot hide", "FCP-0021 remains", "Synthetic fixtures do not close GAP-109")),
        "adr_registered": all(term in adr for term in ("FCF-V2-ADR-038", "Bind Cross-Source Coverage to One Registered Calendar", "cannot select a winning provider")),
        "gap_preserved": "V2-FR-GAP-109" in gaps and "RESEARCH_REQUIRED" in gaps,
        "protocol_registered": "Proposal `FCF-FCP-0038`" in protocol,
        "memory_registered": all(term in memory for term in ("registered same-calendar A-share cross-source", "source-specific", "no source selection")),
        "delivery_files_exist": all((root / path).is_file() for path in (
            "FCF_CURRENT_STATE_FCP_0038_A_SHARE_REGISTERED_SAME_CALENDAR_CROSS_SOURCE_COVERAGE_RECONCILIATION_APP_1_APPROVED.md",
            "FCF_CURRENT_STATE_FCP_0038_A_SHARE_REGISTERED_SAME_CALENDAR_CROSS_SOURCE_COVERAGE_RECONCILIATION_APP_1_DELIVERED.md",
            "docs/FCF_FCP_0038_A_SHARE_REGISTERED_SAME_CALENDAR_CROSS_SOURCE_COVERAGE_RECONCILIATION_APP_1_D1_D6.md",
            "apps/fcp_0038_a_share_registered_same_calendar_cross_source_coverage_reconciliation_app_1/reconciliation.py",
            "tests/fcp_0038_a_share_registered_same_calendar_cross_source_coverage_reconciliation_app_1/test_d1_d6.py",
        )),
        "run_all_wired": "control_center_fcp_0038_a_share_registered_same_calendar_cross_source_coverage_reconciliation_guard.py" in run_all,
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0038 guard failed: " + ",".join(failed))
