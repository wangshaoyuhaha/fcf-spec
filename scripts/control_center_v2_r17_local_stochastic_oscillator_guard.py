...
from apps.v2_r17_local_stochastic_oscillator_foundation_app_1 import (  # noqa: E402
    V2_R17_STOCHASTIC_INDICATOR_BOUNDARY,
)

APP_ROOT = Path("apps/v2_r17_local_stochastic_oscillator_foundation_app_1")
APP_FILES = (
    "__init__.py",
    "acceptance.py",
    "boundary.py",
...
DOCS = tuple(
    Path(f"docs/V2_R17_LOCAL_STOCHASTIC_OSCILLATOR_FOUNDATION_APP_1_D{i}.md")
    for i in range(1, 7)
)
PROHIBITED = (
    "import requests",
...
def build_v2_r17_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
        app_text = "\n".join(
    ...
    boundary = V2_R17_STOCHASTIC_INDICATOR_BOUNDARY
    checks = {
        ...
        "deterministic_stochastic_present": all(
            token in app_text
            for token in (
                "KDJ",
                "ROUND_HALF_EVEN",
...
def main() -> int:
    report = build_v2_r17_guard_report()
    if report["ok"] is not True:
        raise SystemExit("V2-R17 local stochastic oscillator guard failed")
