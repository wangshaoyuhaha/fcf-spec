...
DELIVERY_ID = "FCF-FCP-0070-BTC-PERPETUAL-PAPER-STRESS-DETERMINISTIC-TRIGGER-EVALUATION-APP-1"
MARKER = "FCP 0070 BTC PERPETUAL PAPER STRESS DETERMINISTIC TRIGGER EVALUATION APP 1"
AUTHORITIES = tuple(
    Path(value)
    for value in (
...
def build_fcp_0070_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0070"
        ),
        {},
    )
    expected = {
        "FCF_CURRENT_STATE_FCP_0070_BTC_PERPETUAL_PAPER_STRESS_DETERMINISTIC_TRIGGER_EVALUATION_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0070_BTC_PERPETUAL_PAPER_STRESS_DETERMINISTIC_TRIGGER_EVALUATION_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0070_BTC_PERPETUAL_PAPER_STRESS_DETERMINISTIC_TRIGGER_EVALUATION_APP_1_FINAL.md",
        "docs/FCF_FCP_0070_BTC_PERPETUAL_PAPER_STRESS_DETERMINISTIC_TRIGGER_EVALUATION_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "architecture_registered": all(
            ...
            for term in (
                ...
                "exact typed FCP-0069",
                "Denominator policies reject nonpositive baselines",
            )
        ),
        "adr_registered": all(
            term in adr
            for term in (
                "FCF-V2-ADR-070",
                "Evaluate Closed Stress Triggers With Exact Decimal Arithmetic",
        ...
        "protocol_registered": "Proposal `FCF-FCP-0070`" in protocol,
        "memory_registered": all(
            term in memory
            for term in (
                "deterministic stress-trigger evaluations",
                "Decimal formula execution",
                "no account or execution authority",
            )
        ),
        "delivery_files_exist": all(
            ...
            for path in (
                ...
                "FCF_CURRENT_STATE_FCP_0070_BTC_PERPETUAL_PAPER_STRESS_DETERMINISTIC_TRIGGER_EVALUATION_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0070_BTC_PERPETUAL_PAPER_STRESS_DETERMINISTIC_TRIGGER_EVALUATION_APP_1_D1_D6.md",
                "apps/fcp_0070_btc_perpetual_paper_stress_deterministic_trigger_evaluation_app_1/evaluator.py",
                "tests/fcp_0070_btc_perpetual_paper_stress_deterministic_trigger_evaluation_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": "control_center_fcp_0070_btc_perpetual_paper_stress_deterministic_trigger_evaluation_guard.py"
        in run_all,
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0070 guard failed: " + failures)
