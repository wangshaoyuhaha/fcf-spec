...
def build_parser():
    ...
    parser.description = "Run the FCP-0009 read-only market data diagnostics console."
    parser.add_argument(
        "--language",
        choices=("zh-CN", "en"),
        default="zh-CN",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        runtime = build_market_data_diagnostics_runtime(
        ...
        return 2
    url = f"http://127.0.0.1:{profile.port}/market-data-diagnostics"
    print(f"FCF market data diagnostics console: {url}", flush=True)
    print("Registered local replay only / External activation blocked", flush=True)
    if args.check:
        server.server_close()
        print("Market data diagnostics startup preflight: PASS", flush=True)
        return 0
    try:
        ...
        print("FCF market data diagnostics console stopped by Operator.", flush=True)
    finally:
        server.server_close()
    return 0
