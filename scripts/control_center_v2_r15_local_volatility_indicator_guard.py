...
from apps.v2_r15_local_volatility_indicator_foundation_app_1 import V2_R15_VOLATILITY_INDICATOR_BOUNDARY  # noqa: E402

APP_ROOT = Path("apps/v2_r15_local_volatility_indicator_foundation_app_1")
APP_FILES = (
    "__init__.py",
    "acceptance.py",
    "boundary.py",
...
DOCS = tuple(
    Path(f"docs/V2_R15_LOCAL_VOLATILITY_INDICATOR_FOUNDATION_APP_1_D{i}.md")
    for i in range(1, 7)
)
PROHIBITED = (
    "import requests",
...
def build_v2_r15_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
        app_text = "\n".join(
    ...
    boundary = V2_R15_VOLATILITY_INDICATOR_BOUNDARY
    checks = {
        ...
        "deterministic_volatility_present": all(
            token in app_text
            for token in (
                "TRUE_RANGE",
                "AVERAGE_TRUE_RANGE",
...
def main() -> int:
    report = build_v2_r15_guard_report()
    if report["ok"] is not True:
        raise SystemExit("V2-R15 local volatility indicator guard failed")
