...
DELIVERY_ID = "FCF-FCP-0047-BTC-PERPETUAL-MARGIN-RISK-TIER-EVIDENCE-REGISTRY-APP-1"
...
MARKER = "FCP 0047 BTC PERPETUAL MARGIN RISK TIER EVIDENCE REGISTRY APP 1"
...
def build_fcp_0047_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next((item for item in intake.get("proposals", []) if item.get("proposal_id") == "FCF-FCP-0047"), {})
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0047_BTC_PERPETUAL_MARGIN_RISK_TIER_EVIDENCE_REGISTRY_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0047_BTC_PERPETUAL_MARGIN_RISK_TIER_EVIDENCE_REGISTRY_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0047_BTC_PERPETUAL_MARGIN_RISK_TIER_EVIDENCE_REGISTRY_APP_1_FINAL.md",
        "docs/FCF_FCP_0047_BTC_PERPETUAL_MARGIN_RISK_TIER_EVIDENCE_REGISTRY_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "final_evidence_when_closed": not closed or ((root / "FCF_CURRENT_STATE_FCP_0047_BTC_PERPETUAL_MARGIN_RISK_TIER_EVIDENCE_REGISTRY_APP_1_FINAL.md").is_file() and all(finals) and "ALL CHECKS PASSED" in finals[0]),
        ...
        "architecture_registered": all(term in architecture for term in ("## 80. BTC Perpetual Margin Risk Tier Evidence Registry", "remain contiguous and", "nonoverlapping", "contract-entry hash")),
        "adr_registered": all(term in adr for term in ("FCF-V2-ADR-047", "Resolve BTC Margin Rules Only From Exact Contract-Bound Evidence", "infer missing tiers")),
        "gaps_preserved": all(term in gaps for term in ("V2-FR-GAP-097", "V2-FR-GAP-102", "NOT_IMPLEMENTED", "RESEARCH_REQUIRED")),
        "protocol_registered": "Proposal `FCF-FCP-0047`" in protocol,
        "memory_registered": all(term in memory for term in ("BTC perpetual margin risk-tier registries", "exact FCP-0046 contract", "fail-closed lookup")),
        "delivery_files_exist": all((root / path).is_file() for path in (
            "FCF_CURRENT_STATE_FCP_0047_BTC_PERPETUAL_MARGIN_RISK_TIER_EVIDENCE_REGISTRY_APP_1_APPROVED.md",
            "FCF_CURRENT_STATE_FCP_0047_BTC_PERPETUAL_MARGIN_RISK_TIER_EVIDENCE_REGISTRY_APP_1_DELIVERED.md",
            "docs/FCF_FCP_0047_BTC_PERPETUAL_MARGIN_RISK_TIER_EVIDENCE_REGISTRY_APP_1_D1_D6.md",
            "apps/fcp_0047_btc_perpetual_margin_risk_tier_evidence_registry_app_1/registry.py",
            "tests/fcp_0047_btc_perpetual_margin_risk_tier_evidence_registry_app_1/test_d1_d6.py",
        )),
        "run_all_wired": "control_center_fcp_0047_btc_perpetual_margin_risk_tier_evidence_registry_guard.py" in run_all,
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0047 guard failed: " + ",".join(failed))
