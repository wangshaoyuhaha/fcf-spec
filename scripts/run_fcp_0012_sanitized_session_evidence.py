...
def build_parser():
    ...
    parser.description = "Run the FCP-0012 sanitized session evidence review."
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    review = review_candidate_session_evidence(candidate, registration, evidence)
    if args.review_json:
    ...
    try:
        ...
        runtime = build_sanitized_session_evidence_runtime(
        ...
        return 2
    url = f"http://127.0.0.1:{profile.port}/data-source-session-evidence"
    print(f"FCF sanitized session evidence review: {url}", flush=True)
    print("Read-only / Sanitized artifact / Operator review", flush=True)
    if args.check:
        server.server_close()
        print("FCP-0012 startup preflight: PASS", flush=True)
        return 0
    try:
        ...
        print("FCP-0012 console stopped by Operator.", flush=True)
    finally:
        server.server_close()
    return 0
