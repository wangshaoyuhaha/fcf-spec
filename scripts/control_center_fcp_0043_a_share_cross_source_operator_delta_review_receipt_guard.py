...
DELIVERY_ID = (
    "FCF-FCP-0043-A-SHARE-CROSS-SOURCE-OPERATOR-DELTA-REVIEW-RECEIPT-APP-1"
)
AUTHORITIES = tuple(
    Path(value)
    for value in (
...
MARKER = "FCP 0043 A SHARE CROSS SOURCE OPERATOR DELTA REVIEW RECEIPT APP 1"
...
def build_fcp_0043_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0043"
        ),
        {},
    )
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0043_A_SHARE_CROSS_SOURCE_OPERATOR_DELTA_REVIEW_RECEIPT_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0043_A_SHARE_CROSS_SOURCE_OPERATOR_DELTA_REVIEW_RECEIPT_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0043_A_SHARE_CROSS_SOURCE_OPERATOR_DELTA_REVIEW_RECEIPT_APP_1_FINAL.md",
        "docs/FCF_FCP_0043_A_SHARE_CROSS_SOURCE_OPERATOR_DELTA_REVIEW_RECEIPT_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "architecture_registered": all(
            ...
            for term in (
                ...
                "does not validate or reject evidence",
            )
        ),
        "adr_registered": all(
            term in adr
            for term in (
                "FCF-V2-ADR-043",
                "Separate Review Completion from Evidence Acceptance",
                "treat acknowledgement as evidence acceptance",
            )
        ),
        "gap_preserved": "V2-FR-GAP-109" in gaps and "RESEARCH_REQUIRED" in gaps,
        "protocol_registered": "Proposal `FCF-FCP-0043`" in protocol,
        "memory_registered": all(
            term in memory
            for term in (
                "Operator delta-review receipts with safe review metadata",
                "three closed non-decisional",
                "no evidence",
            )
        ),
        "delivery_files_exist": all(
            ...
            for path in (
                ...
                "FCF_CURRENT_STATE_FCP_0043_A_SHARE_CROSS_SOURCE_OPERATOR_DELTA_REVIEW_RECEIPT_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0043_A_SHARE_CROSS_SOURCE_OPERATOR_DELTA_REVIEW_RECEIPT_APP_1_D1_D6.md",
                "apps/fcp_0043_a_share_cross_source_operator_delta_review_receipt_app_1/receipt.py",
                "tests/fcp_0043_a_share_cross_source_operator_delta_review_receipt_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0043_a_share_cross_source_operator_delta_review_receipt_guard.py"
            in run_all
        ),
...
def main() -> int:
    report = build_fcp_0043_guard_report()
    if report["ok"] is not True:
        failed = sorted(key for key, value in report["checks"].items() if not value)
        raise SystemExit("FCP-0043 guard failed: " + ",".join(failed))
