...
DELIVERY_ID = (
    "FCF-FCP-0039-A-SHARE-CROSS-SOURCE-ARTIFACT-INDEPENDENCE-INTEGRITY-"
    "HARDENING-APP-1"
)
AUTHORITIES = tuple(
    Path(value)
...
MARKER = (
    "FCP 0039 A SHARE CROSS SOURCE ARTIFACT INDEPENDENCE INTEGRITY "
    "HARDENING APP 1"
)
...
def build_fcp_0039_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next((item for item in intake.get("proposals", []) if item.get("proposal_id") == "FCF-FCP-0039"), {})
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0039_A_SHARE_CROSS_SOURCE_ARTIFACT_INDEPENDENCE_INTEGRITY_HARDENING_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0039_A_SHARE_CROSS_SOURCE_ARTIFACT_INDEPENDENCE_INTEGRITY_HARDENING_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0039_A_SHARE_CROSS_SOURCE_ARTIFACT_INDEPENDENCE_INTEGRITY_HARDENING_APP_1_FINAL.md",
        "docs/FCF_FCP_0039_A_SHARE_CROSS_SOURCE_ARTIFACT_INDEPENDENCE_INTEGRITY_HARDENING_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "final_evidence_when_closed": not closed or ((root / "FCF_CURRENT_STATE_FCP_0039_A_SHARE_CROSS_SOURCE_ARTIFACT_INDEPENDENCE_INTEGRITY_HARDENING_APP_1_FINAL.md").is_file() and all(finals) and "ALL CHECKS PASSED" in finals[0]),
        ...
        "architecture_registered": all(term in architecture for term in ("## 72. A-Share Cross-Source Artifact Independence Integrity", "Any digest overlap fails", "does not prove provider corporate independence")),
        "adr_registered": all(term in adr for term in ("FCF-V2-ADR-039", "Bind Cross-Source Roles to Disjoint Artifact Lineage", "different labels as independent")),
        "gap_preserved": "V2-FR-GAP-109" in gaps and "RESEARCH_REQUIRED" in gaps,
        "protocol_registered": "Proposal `FCF-FCP-0039`" in protocol,
        "memory_registered": all(term in memory for term in ("cross-source artifact-independence", "disjoint-lineage proof", "provider-independence research")),
        "delivery_files_exist": all((root / path).is_file() for path in (
            "FCF_CURRENT_STATE_FCP_0039_A_SHARE_CROSS_SOURCE_ARTIFACT_INDEPENDENCE_INTEGRITY_HARDENING_APP_1_APPROVED.md",
            "FCF_CURRENT_STATE_FCP_0039_A_SHARE_CROSS_SOURCE_ARTIFACT_INDEPENDENCE_INTEGRITY_HARDENING_APP_1_DELIVERED.md",
            "docs/FCF_FCP_0039_A_SHARE_CROSS_SOURCE_ARTIFACT_INDEPENDENCE_INTEGRITY_HARDENING_APP_1_D1_D6.md",
            "apps/fcp_0039_a_share_cross_source_artifact_independence_integrity_hardening_app_1/integrity.py",
            "tests/fcp_0039_a_share_cross_source_artifact_independence_integrity_hardening_app_1/test_d1_d6.py",
        )),
        "run_all_wired": "control_center_fcp_0039_a_share_cross_source_artifact_independence_integrity_hardening_guard.py" in run_all,
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0039 guard failed: " + ",".join(failed))
