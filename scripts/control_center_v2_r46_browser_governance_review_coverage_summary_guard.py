...
APP_ROOT = Path(
    "apps/v2_r46_browser_governance_review_coverage_summary_presentation_app_1"
)
APP_FILES = (
    "__init__.py",
    "acceptance.py",
...
DOCS = tuple(
    Path(
        "docs/V2_R46_BROWSER_GOVERNANCE_REVIEW_COVERAGE_SUMMARY_"
        f"PRESENTATION_APP_1_D{i}.md"
    )
    for i in range(1, 7)
)
PROHIBITED = (
...
def build_v2_r46_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
        text = "\n".join(
    ...
    checks = {
        ...
        "browser_integration_present": all(
            ...
            for item in (
                ...
                "Governance Review Coverage Summary",
                "Missing evidence",
            )
        ),
        ...
        and boundary.registered_evidence_only
...
def main() -> int:
    report = build_v2_r46_guard_report()
    if report["ok"] is not True:
        raise SystemExit(
            "V2-R46 browser governance review coverage summary guard failed"
        )
