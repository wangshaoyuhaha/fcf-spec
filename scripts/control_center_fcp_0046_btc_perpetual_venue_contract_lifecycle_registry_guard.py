...
DELIVERY_ID = "FCF-FCP-0046-BTC-PERPETUAL-VENUE-CONTRACT-LIFECYCLE-REGISTRY-APP-1"
...
MARKER = "FCP 0046 BTC PERPETUAL VENUE CONTRACT LIFECYCLE REGISTRY APP 1"
...
def build_fcp_0046_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next((item for item in intake.get("proposals", []) if item.get("proposal_id") == "FCF-FCP-0046"), {})
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0046_BTC_PERPETUAL_VENUE_CONTRACT_LIFECYCLE_REGISTRY_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0046_BTC_PERPETUAL_VENUE_CONTRACT_LIFECYCLE_REGISTRY_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0046_BTC_PERPETUAL_VENUE_CONTRACT_LIFECYCLE_REGISTRY_APP_1_FINAL.md",
        "docs/FCF_FCP_0046_BTC_PERPETUAL_VENUE_CONTRACT_LIFECYCLE_REGISTRY_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "final_evidence_when_closed": not closed or ((root / "FCF_CURRENT_STATE_FCP_0046_BTC_PERPETUAL_VENUE_CONTRACT_LIFECYCLE_REGISTRY_APP_1_FINAL.md").is_file() and all(finals) and "ALL CHECKS PASSED" in finals[0]),
        ...
        "architecture_registered": all(term in architecture for term in ("## 79. BTC Perpetual Venue Contract Lifecycle Registry", "half-open UTC effective interval", "fails closed on")),
        "adr_registered": all(term in adr for term in ("FCF-V2-ADR-046", "Resolve BTC Contract Rules Only by Registered Effective Time", "hard-code one global BTC contract")),
        "gaps_preserved": all(term in gaps for term in ("V2-FR-GAP-096", "V2-FR-GAP-102", "NOT_IMPLEMENTED", "RESEARCH_REQUIRED")),
        "protocol_registered": "Proposal `FCF-FCP-0046`" in protocol,
        "memory_registered": all(term in memory for term in ("BTC perpetual venue contract-lifecycle registries", "half-open effective", "fail-closed point-in-time lookup")),
        "delivery_files_exist": all((root / path).is_file() for path in (
            "FCF_CURRENT_STATE_FCP_0046_BTC_PERPETUAL_VENUE_CONTRACT_LIFECYCLE_REGISTRY_APP_1_APPROVED.md",
            "FCF_CURRENT_STATE_FCP_0046_BTC_PERPETUAL_VENUE_CONTRACT_LIFECYCLE_REGISTRY_APP_1_DELIVERED.md",
            "docs/FCF_FCP_0046_BTC_PERPETUAL_VENUE_CONTRACT_LIFECYCLE_REGISTRY_APP_1_D1_D6.md",
            "apps/fcp_0046_btc_perpetual_venue_contract_lifecycle_registry_app_1/registry.py",
            "tests/fcp_0046_btc_perpetual_venue_contract_lifecycle_registry_app_1/test_d1_d6.py",
        )),
        "run_all_wired": "control_center_fcp_0046_btc_perpetual_venue_contract_lifecycle_registry_guard.py" in run_all,
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0046 guard failed: " + ",".join(failed))
