...
from apps.fcp_0079_a_share_corporate_action_query_policy_lineage_contract_app_1 import (  # noqa: E402
    build_augmented_coverage_matrix,
)


DELIVERY_ID = (
    "FCF-FCP-0079-A-SHARE-CORPORATE-ACTION-QUERY-POLICY-LINEAGE-"
    "CONTRACT-APP-1"
)
MARKER = "FCP 0079 A SHARE CORPORATE ACTION QUERY POLICY LINEAGE CONTRACT APP 1"
MATRIX_HASH = "a522a720a7115b59a8146e675c5e9c26892a98f07b3f5f4557949d834b5b0e08"
AUTHORITIES = tuple(
    Path(value)
    for value in (
...
def build_fcp_0079_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
        ...
        matrix = build_augmented_coverage_matrix(root, evaluated_at_utc="2026-07-23T03:30:00Z")
        readable = True
    ...
    proposal = next((item for item in intake.get("proposals", []) if item.get("proposal_id") == "FCF-FCP-0079"), {})
    expected = {
        "FCF_CURRENT_STATE_FCP_0079_A_SHARE_CORPORATE_ACTION_QUERY_POLICY_LINEAGE_CONTRACT_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0079_A_SHARE_CORPORATE_ACTION_QUERY_POLICY_LINEAGE_CONTRACT_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0079_A_SHARE_CORPORATE_ACTION_QUERY_POLICY_LINEAGE_CONTRACT_APP_1_FINAL.md",
        "docs/FCF_FCP_0079_A_SHARE_CORPORATE_ACTION_QUERY_POLICY_LINEAGE_CONTRACT_APP_1_D1_D6.md",
    }
    row = next((item for item in matrix.rows if item.requirement.gap_id == "V2-FR-GAP-089"), None) if matrix else None
    gap_row = next((line for line in gaps.splitlines() if "| V2-FR-GAP-089 |" in line), "")
    checks = {
        ...
        "adr_registered": all(
            ...
            for term in (
                "FCF-V2-ADR-079",
                "Separate Corporate Actions From Price Query Policy",
                "overwrite raw prices",
            )
        ),
        "gap_preserved": "| NOT_IMPLEMENTED |" in gap_row,
        "gap_observation_registered": "FCP-0079 is approved" in gaps and "price-lineage gap" in gaps,
        "protocol_registered": "Proposal `FCF-FCP-0079`" in protocol,
        "memory_registered": "corporate-action and query-policy lineage contracts" in memory,
        "run_all_wired": "control_center_fcp_0079_a_share_corporate_action_query_policy_lineage_contract_guard.py" in run_all,
        "matrix_hash_exact": matrix is not None and matrix.matrix_hash == MATRIX_HASH,
        "price_lineage_foundation_covered": row is not None
