...
DELIVERY_ID = (
    "FCF-FCP-0040-A-SHARE-SAME-CALENDAR-CROSS-SOURCE-FIELD-DELTA-"
    "DIAGNOSTIC-APP-1"
)
AUTHORITIES = tuple(
    Path(value)
...
MARKER = "FCP 0040 A SHARE SAME CALENDAR CROSS SOURCE FIELD DELTA DIAGNOSTIC APP 1"
...
def build_fcp_0040_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0040"
        ),
        {},
    )
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0040_A_SHARE_SAME_CALENDAR_CROSS_SOURCE_FIELD_DELTA_DIAGNOSTIC_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0040_A_SHARE_SAME_CALENDAR_CROSS_SOURCE_FIELD_DELTA_DIAGNOSTIC_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0040_A_SHARE_SAME_CALENDAR_CROSS_SOURCE_FIELD_DELTA_DIAGNOSTIC_APP_1_FINAL.md",
        "docs/FCF_FCP_0040_A_SHARE_SAME_CALENDAR_CROSS_SOURCE_FIELD_DELTA_DIAGNOSTIC_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "adr_registered": all(
            ...
            for term in (
                "FCF-V2-ADR-040",
                "Keep Cross-Source Delta Diagnostics Non-Decisional",
                "infer a preferred provider",
            )
        ),
        "gap_preserved": "V2-FR-GAP-109" in gaps and "RESEARCH_REQUIRED" in gaps,
        "protocol_registered": "Proposal `FCF-FCP-0040`" in protocol,
        "memory_registered": all(
            term in memory
            for term in (
                "same-calendar cross-source field-delta diagnostics",
                "exact overlap-only",
                "no threshold, ranking",
            )
        ),
        "delivery_files_exist": all(
            ...
            for path in (
                ...
                "FCF_CURRENT_STATE_FCP_0040_A_SHARE_SAME_CALENDAR_CROSS_SOURCE_FIELD_DELTA_DIAGNOSTIC_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0040_A_SHARE_SAME_CALENDAR_CROSS_SOURCE_FIELD_DELTA_DIAGNOSTIC_APP_1_D1_D6.md",
                "apps/fcp_0040_a_share_same_calendar_cross_source_field_delta_diagnostic_app_1/diagnostic.py",
                "tests/fcp_0040_a_share_same_calendar_cross_source_field_delta_diagnostic_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0040_a_share_same_calendar_cross_source_field_delta_diagnostic_guard.py"
            in run_all
        ),
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0040 guard failed: " + ",".join(failed))
