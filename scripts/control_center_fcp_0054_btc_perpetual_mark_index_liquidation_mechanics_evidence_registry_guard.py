...
DELIVERY_ID = (
    "FCF-FCP-0054-BTC-PERPETUAL-MARK-INDEX-LIQUIDATION-MECHANICS-EVIDENCE-REGISTRY-APP-1"
)
AUTHORITIES = tuple(
    Path(value)
    for value in (
...
MARKER = (
    "FCP 0054 BTC PERPETUAL MARK INDEX LIQUIDATION MECHANICS EVIDENCE REGISTRY APP 1"
)
...
def build_fcp_0054_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0054"
        ),
        {},
    )
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0054_BTC_PERPETUAL_MARK_INDEX_LIQUIDATION_MECHANICS_EVIDENCE_REGISTRY_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0054_BTC_PERPETUAL_MARK_INDEX_LIQUIDATION_MECHANICS_EVIDENCE_REGISTRY_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0054_BTC_PERPETUAL_MARK_INDEX_LIQUIDATION_MECHANICS_EVIDENCE_REGISTRY_APP_1_FINAL.md",
        "docs/FCF_FCP_0054_BTC_PERPETUAL_MARK_INDEX_LIQUIDATION_MECHANICS_EVIDENCE_REGISTRY_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "architecture_registered": all(
            ...
            for term in (
                ...
                "Partial-liquidation tiers begin at zero",
        ...
        "adr_registered": all(
            ...
            for term in (
                "FCF-V2-ADR-054",
                "Preserve BTC Liquidation Mechanics As Versioned Evidence Before Calculation",
                "derive liquidation rules from observed prices",
            )
        ),
        "gaps_preserved": all(
            term in gaps
        ...
        "protocol_registered": "Proposal `FCF-FCP-0054`" in protocol,
        "memory_registered": all(
            term in memory
            for term in (
                "BTC perpetual mark-index-liquidation mechanics registries",
                "partial-liquidation tiers",
                "no price, liquidation, or execution calculation",
            )
        ),
        "delivery_files_exist": all(
            ...
            for path in (
                ...
                "FCF_CURRENT_STATE_FCP_0054_BTC_PERPETUAL_MARK_INDEX_LIQUIDATION_MECHANICS_EVIDENCE_REGISTRY_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0054_BTC_PERPETUAL_MARK_INDEX_LIQUIDATION_MECHANICS_EVIDENCE_REGISTRY_APP_1_D1_D6.md",
                "apps/fcp_0054_btc_perpetual_mark_index_liquidation_mechanics_evidence_registry_app_1/registry.py",
                "tests/fcp_0054_btc_perpetual_mark_index_liquidation_mechanics_evidence_registry_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0054_btc_perpetual_mark_index_liquidation_mechanics_evidence_registry_guard.py"
            in run_all
        ),
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0054 guard failed: " + ",".join(failed))
