...
from apps.fcp_0087_btc_registered_local_export_operator_review_receipt_app_1 import (  # noqa: E402
...
DELIVERY_ID = "FCF-FCP-0087-BTC-REGISTERED-LOCAL-EXPORT-OPERATOR-REVIEW-RECEIPT-APP-1"
MARKER = "FCP 0087 BTC REGISTERED LOCAL EXPORT OPERATOR REVIEW RECEIPT APP 1"
CONTRACT_PATH = "apps/fcp_0087_btc_registered_local_export_operator_review_receipt_app_1/contracts.py"
CONTRACT_SHA = "c96d72ec08dc195ae0ab7cd2a603ba0e5374ab83e288e29555bb399fc5bd320b"
REFERENCE_RECEIPT_HASH = "8dd43b2d7426e01342bbba25a6b5b8dc9cecd6b11b65a7e9a010490d0765fbc3"
REFERENCE_OUTPUT_SHA = "d1e23574e88519831059e3459450be6add1e98edd2c657b2d5cebce282d13143"
...
def build_fcp_0087_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
        ...
        reference = build_reference_receipt()
        reference_output = render_operator_review_receipt_json(reference).encode("ascii")
        readable = True
    ...
    proposal = next(
        (item for item in intake.get("proposals", []) if item.get("proposal_id") == "FCF-FCP-0087"),
        {},
    )
    expected = {
        "FCF_CURRENT_STATE_FCP_0087_BTC_REGISTERED_LOCAL_EXPORT_OPERATOR_REVIEW_RECEIPT_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0087_BTC_REGISTERED_LOCAL_EXPORT_OPERATOR_REVIEW_RECEIPT_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0087_BTC_REGISTERED_LOCAL_EXPORT_OPERATOR_REVIEW_RECEIPT_APP_1_FINAL.md",
        "docs/FCF_FCP_0087_BTC_REGISTERED_LOCAL_EXPORT_OPERATOR_REVIEW_RECEIPT_APP_1_D1_D6.md",
    }
    ...
    prohibited = ("import requests", "import socket", "import urllib", "import websocket", "import ccxt", "import binance")
    checks = {
        ...
        "final_evidence_when_closed": not closed or ((root / final).is_file() and all(finals) and "ALL CHECKS PASSED" in finals[0]),
        ...
        "architecture_registered": "## 120. BTC Registered Local Export Operator Review Receipt" in architecture,
        "adr_registered": "FCF-V2-ADR-087" in adr and "Record Operator Review Without Granting Authority" in adr,
        "gap_preserved": "| RESEARCH_REQUIRED |" in gap_row and "FCP-0087 is approved" in gaps,
        "protocol_registered": "Proposal `FCF-FCP-0087`" in protocol and "FCP-0086" in protocol,
        "memory_registered": "FCP-0087 preserves explicit Operator review" in memory,
        "run_all_wired": "control_center_fcp_0087_btc_registered_local_export_operator_review_receipt_guard.py" in run_all,
        "contract_hash_exact": hashlib.sha256(contract_bytes).hexdigest() == CONTRACT_SHA,
        "reference_receipt_hash_exact": reference is not None and reference.receipt_hash == REFERENCE_RECEIPT_HASH,
        ...
        and reference.operator_review_completed is True
        and reference.evidence_approved is False
        and reference.evidence_rejected is False
        ...
        "contract_reuses_packet_authority": "BTCLocalExportOperatorReviewPacket" in contract_text
        and "fcp_0086_btc_registered_local_export" in contract_text
        and not any(term in contract_text for term in prohibited),
        "delivery_files_exist": all((root / path).is_file() for path in expected if "DELIVERED" not in path and "FINAL" not in path)
