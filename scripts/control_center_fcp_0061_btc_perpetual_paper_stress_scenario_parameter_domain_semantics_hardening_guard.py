...
DELIVERY_ID = (
    "FCF-FCP-0061-BTC-PERPETUAL-PAPER-STRESS-SCENARIO-PARAMETER-DOMAIN-SEMANTICS-HARDENING-APP-1"
)
AUTHORITIES = tuple(
    Path(value)
    for value in (
...
MARKER = (
    "FCP 0061 BTC PERPETUAL PAPER STRESS SCENARIO PARAMETER DOMAIN SEMANTICS HARDENING APP 1"
)
...
def build_fcp_0061_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0061"
        ),
        {},
    )
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0061_BTC_PERPETUAL_PAPER_STRESS_SCENARIO_PARAMETER_DOMAIN_SEMANTICS_HARDENING_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0061_BTC_PERPETUAL_PAPER_STRESS_SCENARIO_PARAMETER_DOMAIN_SEMANTICS_HARDENING_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0061_BTC_PERPETUAL_PAPER_STRESS_SCENARIO_PARAMETER_DOMAIN_SEMANTICS_HARDENING_APP_1_FINAL.md",
        "docs/FCF_FCP_0061_BTC_PERPETUAL_PAPER_STRESS_SCENARIO_PARAMETER_DOMAIN_SEMANTICS_HARDENING_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "architecture_registered": all(
            ...
            for term in (
                "## 94. BTC Perpetual Paper Stress Scenario Parameter Domain Semantics Hardening",
                "kind-specific parameter domains",
        ...
        "adr_registered": all(
            ...
            for term in (
                "FCF-V2-ADR-061",
                "Use Kind-Specific Domains For Stress Scenario Parameters",
                "positive integral counts",
            )
        ),
        "gaps_preserved": all(
            term in gaps
        ...
        "protocol_registered": "Proposal `FCF-FCP-0061`" in protocol,
        "memory_registered": all(
            term in memory
            for term in (
                "stress-scenario parameter domain hardening",
                "FCP-0056 and FCP-0057",
        ...
        "delivery_files_exist": all(
            ...
            for path in (
                ...
                "FCF_CURRENT_STATE_FCP_0061_BTC_PERPETUAL_PAPER_STRESS_SCENARIO_PARAMETER_DOMAIN_SEMANTICS_HARDENING_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0061_BTC_PERPETUAL_PAPER_STRESS_SCENARIO_PARAMETER_DOMAIN_SEMANTICS_HARDENING_APP_1_D1_D6.md",
                "apps/fcp_0061_btc_perpetual_paper_stress_scenario_parameter_domain_semantics_hardening_app_1/hardening.py",
                "tests/fcp_0061_btc_perpetual_paper_stress_scenario_parameter_domain_semantics_hardening_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0061_btc_perpetual_paper_stress_scenario_parameter_domain_semantics_hardening_guard.py"
            in run_all
        ),
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0061 guard failed: " + ",".join(failed))
