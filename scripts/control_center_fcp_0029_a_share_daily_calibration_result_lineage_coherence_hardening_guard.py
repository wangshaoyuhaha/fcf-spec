...
DELIVERY_ID = "FCF-FCP-0029-A-SHARE-DAILY-CALIBRATION-RESULT-LINEAGE-COHERENCE-HARDENING-APP-1"
AUTHORITIES = (
...
APPROVAL_START = "<!-- FCP 0029 A SHARE DAILY CALIBRATION RESULT LINEAGE COHERENCE HARDENING APP 1 APPROVAL START -->"
APPROVAL_END = "<!-- FCP 0029 A SHARE DAILY CALIBRATION RESULT LINEAGE COHERENCE HARDENING APP 1 APPROVAL END -->"
LOCK_START = "<!-- FCP 0029 A SHARE DAILY CALIBRATION RESULT LINEAGE COHERENCE HARDENING APP 1 LOCK START -->"
LOCK_END = "<!-- FCP 0029 A SHARE DAILY CALIBRATION RESULT LINEAGE COHERENCE HARDENING APP 1 LOCK END -->"
FINAL_START = "<!-- FCP 0029 A SHARE DAILY CALIBRATION RESULT LINEAGE COHERENCE HARDENING APP 1 FINAL START -->"
FINAL_END = "<!-- FCP 0029 A SHARE DAILY CALIBRATION RESULT LINEAGE COHERENCE HARDENING APP 1 FINAL END -->"
...
def build_fcp_0029_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0029"
        ),
        {},
    )
    markers = (
        "calibration requires typed observations",
        "calibration observations require unique deterministic order",
        "calibration observations exceed the point-in-time boundary",
        "observation source lineage disagrees with RAW manifest",
        "calibration manifest digest lineage disagrees",
        "calibration manifest artifact lineage disagrees",
        "calibration manifest counts disagree with observations",
    ...
    checks = {
        ...
        "calibration_coherence": all(marker in contracts for marker in markers),
        "delivery_files_exist": all(
            (root / path).is_file()
            for path in (
                "FCF_CURRENT_STATE_FCP_0029_A_SHARE_DAILY_CALIBRATION_RESULT_LINEAGE_COHERENCE_HARDENING_APP_1_APPROVED.md",
                "FCF_CURRENT_STATE_FCP_0029_A_SHARE_DAILY_CALIBRATION_RESULT_LINEAGE_COHERENCE_HARDENING_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0029_A_SHARE_DAILY_CALIBRATION_RESULT_LINEAGE_COHERENCE_HARDENING_APP_1_D1_D6.md",
                "tests/fcp_0029_a_share_daily_calibration_result_lineage_coherence_hardening_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0029_a_share_daily_calibration_result_lineage_coherence_hardening_guard.py"
            in run_all
        ),
...
def main() -> int:
    report = build_fcp_0029_guard_report()
    if report["ok"] is not True:
        failed = ",".join(
            sorted(name for name, value in report["checks"].items() if not value)
        )
        raise SystemExit("FCP-0029 guard failed: " + failed)
