...
DELIVERY_ID = (
    "FCF-FCP-0037-A-SHARE-REGISTERED-EXPECTED-TRADING-DATE-ARTIFACT-"
    "PROFILE-APP-1"
)
AUTHORITIES = tuple(
    Path(value)
...
MARKER = (
    "FCP 0037 A SHARE REGISTERED EXPECTED TRADING DATE ARTIFACT PROFILE APP 1"
)
...
def build_fcp_0037_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0037"
        ),
        {},
    )
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0037_A_SHARE_REGISTERED_EXPECTED_TRADING_DATE_ARTIFACT_PROFILE_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0037_A_SHARE_REGISTERED_EXPECTED_TRADING_DATE_ARTIFACT_PROFILE_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0037_A_SHARE_REGISTERED_EXPECTED_TRADING_DATE_ARTIFACT_PROFILE_APP_1_FINAL.md",
        "docs/FCF_FCP_0037_A_SHARE_REGISTERED_EXPECTED_TRADING_DATE_ARTIFACT_PROFILE_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "architecture_registered": all(
            ...
            for term in (
                ...
                "source revision",
                "Weekday, holiday",
                "Synthetic validation does not close GAP-107",
            )
        ),
        "adr_registered": all(
            term in adr
            for term in (
                "FCF-V2-ADR-037",
                "Make Expected Trading Dates a Versioned Registered Artifact",
                "Future revisions fail closed",
                "infer Monday through Friday",
            )
        ),
        "gap_preserved": "V2-FR-GAP-107" in gaps and "RESEARCH_REQUIRED" in gaps,
        "protocol_registered": "Proposal `FCF-FCP-0037`" in protocol,
        "memory_registered": all(
            term in memory
            for term in (
                "registered A-share expected trading-date artifact profiles",
                "point-in-time availability lineage",
        ...
        "delivery_files_exist": all(
            ...
            for path in (
                ...
                "FCF_CURRENT_STATE_FCP_0037_A_SHARE_REGISTERED_EXPECTED_TRADING_DATE_ARTIFACT_PROFILE_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0037_A_SHARE_REGISTERED_EXPECTED_TRADING_DATE_ARTIFACT_PROFILE_APP_1_D1_D6.md",
                "apps/fcp_0037_a_share_registered_expected_trading_date_artifact_profile_app_1/contracts.py",
                "apps/fcp_0037_a_share_registered_expected_trading_date_artifact_profile_app_1/loader.py",
                "tests/fcp_0037_a_share_registered_expected_trading_date_artifact_profile_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0037_a_share_registered_expected_trading_date_artifact_profile_guard.py"
            in run_all
        ),
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0037 guard failed: " + ",".join(failed))
