...
DELIVERY_ID = "FCF-FCP-0063-BTC-PERPETUAL-PAPER-STRESS-EVALUATION-OPERAND-SCHEMA-REGISTRY-APP-1"
AUTHORITIES = tuple(
    Path(value)
    for value in (
...
MARKER = "FCP 0063 BTC PERPETUAL PAPER STRESS EVALUATION OPERAND SCHEMA REGISTRY APP 1"
...
def build_fcp_0063_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0063"
        ),
        {},
    )
    expected = {
        "FCF_CURRENT_STATE_FCP_0063_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_OPERAND_SCHEMA_REGISTRY_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0063_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_OPERAND_SCHEMA_REGISTRY_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0063_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_OPERAND_SCHEMA_REGISTRY_APP_1_FINAL.md",
        "docs/FCF_FCP_0063_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_OPERAND_SCHEMA_REGISTRY_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "architecture_registered": all(
            ...
            for term in (
                "## 96. BTC Perpetual Paper Stress Evaluation Operand Schema Registry",
                "one exact typed FCP-0062 extended-readiness",
                "baseline-current evidence requirements",
            )
        ),
        "adr_registered": all(
            term in adr
            for term in (
                "FCF-V2-ADR-063",
                "Register Complete Stress Evaluation Operands Before Direction",
                "cannot infer a price gap",
            )
        ),
        "gaps_preserved": all(
            term in gaps
        ...
        "protocol_registered": "Proposal `FCF-FCP-0063`" in protocol,
        "memory_registered": all(
            term in memory
            for term in (
                "stress-evaluation operand-schema registries",
                "FCP-0062 lineage",
                "paired baseline-current",
            )
        ),
        "delivery_files_exist": all(
            ...
            for path in (
                ...
                "FCF_CURRENT_STATE_FCP_0063_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_OPERAND_SCHEMA_REGISTRY_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0063_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_OPERAND_SCHEMA_REGISTRY_APP_1_D1_D6.md",
                "apps/fcp_0063_btc_perpetual_paper_stress_evaluation_operand_schema_registry_app_1/registry.py",
                "tests/fcp_0063_btc_perpetual_paper_stress_evaluation_operand_schema_registry_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0063_btc_perpetual_paper_stress_evaluation_operand_schema_registry_guard.py"
            in run_all
        ),
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0063 guard failed: " + failures)
