...
DELIVERY_ID = (
    "FCF-FCP-0060-BTC-PERPETUAL-PAPER-STRESS-EVALUATION-READINESS-COHERENCE-GATE-APP-1"
)
AUTHORITIES = tuple(
    Path(value)
    for value in (
...
MARKER = (
    "FCP 0060 BTC PERPETUAL PAPER STRESS EVALUATION READINESS COHERENCE GATE APP 1"
)
...
def build_fcp_0060_guard_report(root: Path = ROOT) -> dict[str, object]:
    try:
    ...
    proposal = next(
        (
            ...
            if item.get("proposal_id") == "FCF-FCP-0060"
        ),
        {},
    )
    expected_evidence = {
        "FCF_CURRENT_STATE_FCP_0060_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_READINESS_COHERENCE_GATE_APP_1_APPROVED.md",
        "FCF_CURRENT_STATE_FCP_0060_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_READINESS_COHERENCE_GATE_APP_1_DELIVERED.md",
        "FCF_CURRENT_STATE_FCP_0060_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_READINESS_COHERENCE_GATE_APP_1_FINAL.md",
        "docs/FCF_FCP_0060_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_READINESS_COHERENCE_GATE_APP_1_D1_D6.md",
    }
    checks = {
        ...
        "architecture_registered": all(
            ...
            for term in (
                ...
                "snapshot hashes, venue",
                "monotonic effective or as-of UTC lineage",
            )
        ),
        "adr_registered": all(
            term in adr
            for term in (
                "FCF-V2-ADR-060",
                "Require Coherent Stress Evaluation Readiness Evidence",
        ...
        "protocol_registered": "Proposal `FCF-FCP-0060`" in protocol,
        "memory_registered": all(
            term in memory
            for term in (
                "stress-evaluation readiness coherence gates",
                "FCP-0055, FCP-0057, and FCP-0059",
        ...
        "delivery_files_exist": all(
            ...
            for path in (
                ...
                "FCF_CURRENT_STATE_FCP_0060_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_READINESS_COHERENCE_GATE_APP_1_DELIVERED.md",
                "docs/FCF_FCP_0060_BTC_PERPETUAL_PAPER_STRESS_EVALUATION_READINESS_COHERENCE_GATE_APP_1_D1_D6.md",
                "apps/fcp_0060_btc_perpetual_paper_stress_evaluation_readiness_coherence_gate_app_1/gate.py",
                "tests/fcp_0060_btc_perpetual_paper_stress_evaluation_readiness_coherence_gate_app_1/test_d1_d6.py",
            )
        ),
        "run_all_wired": (
            "control_center_fcp_0060_btc_perpetual_paper_stress_evaluation_readiness_coherence_gate_guard.py"
            in run_all
        ),
...
def main() -> int:
    ...
    if report["ok"] is not True:
        ...
        raise SystemExit("FCP-0060 guard failed: " + ",".join(failed))
