...
def test_d2_schema_declares_boundaries():
...
def test_d2_schema_forbids_review_required_and_circuit_break_downgrade():
    text = SCHEMA_DOC.read_text(encoding='utf-8')
    assert 'REVIEW_REQUIRED must not auto-pass' in text
    assert 'CIRCUIT_BREAK must not downgrade' in text


def test_d2_schema_requires_operator_review_routing():
    text = SCHEMA_DOC.read_text(encoding='utf-8')
    for marker in ['REVIEW_REQUIRED', 'CIRCUIT_BREAK', 'conflict_signals', 'missing_required_fields', 'unsafe_permissions']:
        assert marker in text
    assert 'must route to operator review' in text
...
def test_d2_fixture_preserves_blocking_metadata():
    packet = json.loads(FIXTURE.read_text(encoding='utf-8'))
    ...
    assert packet['conflict_signals']
