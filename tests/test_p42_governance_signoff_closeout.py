...
def test_p42_governance_signoff_closeout_uses_966_baseline():
 result = build_p42_governance_signoff_closeout()
 assert result["phase"] == "P42"
 assert result["step_range"] == "D10-D12"
 assert result["baseline_tests"] == 966
 assert result["signoff_closeout_ready"] is True
 assert result["operator_review_required"] is True

def test_p42_signoff_final_packet_preserves_artifacts():
 result = build_p42_signoff_final_packet()
...
def test_p42_signoff_completion_gate_blocks_release_deploy():
 result = build_p42_signoff_completion_gate()
 assert result["p42_complete"] is True
