from backtest_review_app.handoff import (
    APP_ID,
    COMPLETED_STAGES,
    STAGE_ID,
    build_backtest_review_closeout_summary,
    build_backtest_review_workflow_handoff,
    is_valid_backtest_review_closeout_summary,
    is_valid_backtest_review_workflow_handoff,
    validate_backtest_review_closeout_summary,
    validate_backtest_review_workflow_handoff,
)


def test_workflow_handoff_identity_and_completed_stages():
    handoff = build_backtest_review_workflow_handoff(generated_at_utc="2026-01-01T00:00:00+00:00")

    assert handoff.app_id == APP_ID
    assert handoff.stage_id == STAGE_ID
    assert handoff.completed_stages == COMPLETED_STAGES
    assert handoff.branch_name == "sidecar-backtest-review-app-1"


def test_workflow_handoff_safety_flags_forbid_execution_accounts_and_connections():
    handoff = build_backtest_review_workflow_handoff(generated_at_utc="2026-01-01T00:00:00+00:00")
    ...
    assert flags["real_position_access_allowed"] is False
    assert flags["broker_connection_allowed"] is False
    assert flags["exchange_connection_allowed"] is False


def test_workflow_handoff_forbids_profit_guarantee_prediction_release_and_deploy():
    handoff = build_backtest_review_workflow_handoff(generated_at_utc="2026-01-01T00:00:00+00:00")

    assert "trade_instruction" in handoff.downstream_forbidden_uses
    assert "profit_guarantee" in handoff.downstream_forbidden_uses
    assert "future_return_prediction" in handoff.downstream_forbidden_uses
...
def test_valid_workflow_handoff_passes_validation():
    ...
    assert validate_backtest_review_workflow_handoff(handoff) == []
    assert is_valid_backtest_review_workflow_handoff(handoff) is True
    assert handoff.to_dict()["merge_review_required"] is True


def test_closeout_summary_requires_operator_merge_review():
    summary = build_backtest_review_closeout_summary(final_pytest_expected="1334 passed")
...
def test_valid_closeout_summary_passes_validation():
    ...
    assert validate_backtest_review_closeout_summary(summary) == []
    assert is_valid_backtest_review_closeout_summary(summary) is True
    assert summary.to_dict()["final_validation_expected"] == "ALL CHECKS PASSED"


def test_closeout_summary_no_execution_receipt_forbids_actions_predictions_and_guarantees():
    summary = build_backtest_review_closeout_summary(final_pytest_expected="1334 passed")
    ...
    assert receipt["real_account_access_allowed"] is False
    assert receipt["real_position_access_allowed"] is False
    assert receipt["automatic_position_sizing_allowed"] is False
    assert receipt["automatic_portfolio_action_allowed"] is False
    assert receipt["future_return_prediction_allowed"] is False
    assert receipt["guaranteed_performance_claim_allowed"] is False
