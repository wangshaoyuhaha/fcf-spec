...
def test_p41_governance_compliance_dossier_handoff_uses_951_baseline():
 result = build_p41_governance_compliance_dossier_handoff()
 assert result["phase"] == "P41"
 assert result["step_range"] == "D7-D9"
 assert result["baseline_tests"] == 951
 assert result["compliance_dossier_handoff_ready"] is True
 assert result["operator_review_required"] is True

def test_p41_compliance_dossier_operator_packet_blocks_real_actions():
 result = build_p41_compliance_dossier_operator_packet()
...
def test_p41_compliance_dossier_next_gate_blocks_release_deploy():
 result = build_p41_compliance_dossier_next_gate()
 assert result["ready_for_p41_d10_d12"] is True
