...
def _sample_paper_order():
    return {
        ...
        "price": "60050.5",
        "time_in_force": "gtc",
        "source": "unit_test_policy_deny_templates",
        "correlation_id": "p6-d4-policy-deny-template",
        "metadata": {
            "note": "paper only",
        },
    }
...
def test_render_paper_policy_deny_response_is_stable_and_safe():
    adapter_response = _policy_deny_response()
    result = render_paper_policy_deny_response(adapter_response)

    assert adapter_response["http_status"] == 422
    assert adapter_response["body"]["error"]["type"] == "PolicyDeny"
    ...
    assert "risk" in result["fields"]["error_message"]
...
def test_render_user_response_routes_policy_deny_to_policy_template():
...
def test_direct_error_template_still_handles_policy_deny_as_plain_error_when_called_directly():
    result = render_paper_execution_error_response(_policy_deny_response())
...
def test_safety_refusal_is_separate_from_policy_deny():
