...
from btc_finance_platform.paper_release_completion_receipt import build_paper_release_completion_receipt_closeout_checkpoint
from btc_finance_platform.paper_release_completion_receipt import build_paper_release_completion_receipt_export_packet
from btc_finance_platform.paper_release_completion_receipt import build_paper_release_completion_receipt_handoff_packet


def test_p26_d7_completion_receipt_export_packet_is_local_static_read_only():
...
def test_p26_d8_completion_receipt_closeout_checkpoint_passes_completion_and_safety_gates():
    ...
    assert closeout["receipt_item_count"] == 12
    assert closeout["operator_review_required"] is True


def test_p26_d9_completion_receipt_handoff_packet_ready_for_receipt_archive():
    handoff = build_paper_release_completion_receipt_handoff_packet()
    assert handoff["ok"] is True
    assert handoff["handoff_status"] == "READY_FOR_RECEIPT_ARCHIVE"
    assert handoff["next_phase_candidate"] == "P27 Paper Release Final Verification"
