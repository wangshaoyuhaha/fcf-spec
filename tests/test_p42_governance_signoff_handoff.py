...
def test_p42_governance_signoff_handoff_uses_963_baseline():
 result = build_p42_governance_signoff_handoff()
 assert result["phase"] == "P42"
 assert result["step_range"] == "D7-D9"
 assert result["baseline_tests"] == 963
 assert result["signoff_handoff_ready"] is True
 assert result["operator_review_required"] is True

def test_p42_signoff_operator_packet_blocks_real_actions():
 result = build_p42_signoff_operator_packet()
...
def test_p42_signoff_next_gate_blocks_release_deploy():
 result = build_p42_signoff_next_gate()
 assert result["ready_for_p42_d10_d12"] is True
