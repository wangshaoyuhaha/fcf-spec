...
def test_current_state_manifest_records_exact_v2_r21_final_state():
    manifest = load_manifest(ROOT)
    truth = manifest["current_truth"]

    assert truth == V2_R21_FINAL_STATE
    assert manifest["roadmap"] == V2_R21_FINAL_ROADMAP
...
def test_future_status_vocabulary_is_closed_and_excluded_gaps_are_preserved():
    ...
    assert all(line in gap for line in GAP_ROADMAP_R21_FINAL_LINES)
...
def test_manifest_is_deterministic_json_and_historical_order_is_not_current():
    ...
    assert parsed["current_truth"]["next_product_phase_approval"] == (
        V2_R21_FINAL_STATE["next_product_phase_approval"]
    )
