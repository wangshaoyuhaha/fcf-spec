...
def test_current_state_manifest_records_exact_fcp_0001_final_state():
    manifest = load_manifest(ROOT)
    truth = manifest["current_truth"]

    assert truth == FCP_0001_FINAL_STATE
    assert manifest["roadmap"] == FCP_0001_FINAL_ROADMAP
...
def test_manifest_is_deterministic_json_and_historical_order_is_not_current():
    ...
    assert parsed["current_truth"]["next_product_phase_approval"] == (
        FCP_0001_FINAL_STATE["next_product_phase_approval"]
    )
