...
def test_current_state_manifest_records_exact_v2_r16_final_state():
    manifest = load_manifest(ROOT)
    truth = manifest["current_truth"]

    assert truth == V2_R16_FINAL_STATE
    assert manifest["roadmap"] == V2_R16_FINAL_ROADMAP
...
def test_future_status_vocabulary_is_closed_and_excluded_gaps_are_preserved():
    ...
    assert all(line in gap for line in GAP_ROADMAP_R16_FINAL_LINES)
