...
ACCEPTANCE_DOC = Path("docs/76_p8_portfolio_guarded_paper_execution_acceptance.md")


def test_p8_portfolio_acceptance_doc_exists():
    assert ACCEPTANCE_DOC.exists()


def test_p8_portfolio_acceptance_doc_mentions_phase8_days_and_artifacts():
    text = ACCEPTANCE_DOC.read_text(encoding="utf-8")

    for day in ["P8-D1", "P8-D2", "P8-D3", "P8-D4", "P8-D5", "P8-D6", "P8-D7"]:
        assert day in text
...
def test_p8_portfolio_acceptance_doc_mentions_cases_and_response_types():
    text = ACCEPTANCE_DOC.read_text(encoding="utf-8")
...
def test_p8_portfolio_acceptance_smoke_still_completes():
    result = run_smoke()
...
def test_p8_portfolio_acceptance_safe_boundary_from_smoke():
    result = run_smoke()
...
def test_p8_portfolio_acceptance_doc_keeps_safety_boundaries():
    text = ACCEPTANCE_DOC.read_text(encoding="utf-8")
