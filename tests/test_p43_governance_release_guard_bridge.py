...
def test_p43_release_guard_bridge_uses_baseline():
    result = build_p43_governance_release_guard_bridge()
    assert result["phase"] == "P43"
    assert result["step_range"] == "D1-D3"
    assert result["baseline_tests"] == 969
    assert result["release_guard_bridge_ready"] is True
    assert result["operator_review_required"] is True

def test_p43_release_guard_boundary_bridge_blocks_real_actions():
    result = build_p43_release_guard_boundary_bridge()
...
def test_p43_release_guard_gate_bridge_blocks_release_deploy():
    result = build_p43_release_guard_gate_bridge()
    assert result["ready_for_p43_d4_d6"] is True
