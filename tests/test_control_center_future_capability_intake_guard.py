...
def test_registered_proposals_are_durable_and_non_authorizing():
    ...
    assert data["next_proposal_sequence"] == 105
    assert {
    ...
    fcp_0104 = next(
        item for item in data["proposals"] if item["proposal_id"] == "FCF-FCP-0104"
    )
    assert fcp_0104["phase_id"] == "NONE"
