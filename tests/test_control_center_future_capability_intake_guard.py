...
def test_registered_proposals_are_durable_and_non_authorizing():
    ...
    assert data["next_proposal_sequence"] == 101
    assert {
    ...
    assert all(
        ...
        if item["proposal_id"] != "FCF-FCP-0100"
    )
    decisions = {
    ...
    assert decisions["FCF-FCP-0100"] == "APPROVED"
