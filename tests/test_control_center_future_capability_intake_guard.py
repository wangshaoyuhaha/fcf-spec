...
def test_registered_proposals_are_durable_and_non_authorizing():
    ...
    assert data["next_proposal_sequence"] == 97
    assert {
    ...
    assert all(
        item["phase_id"] == (
            ...
            if item["proposal_id"] == "FCF-FCP-0096"
            else "NONE"
        )
    ...
    assert decisions["FCF-FCP-0096"] == "APPROVED"
