...
def test_registered_proposals_are_durable_and_non_authorizing():
    ...
    assert data["next_proposal_sequence"] == 99
    assert {
    ...
    assert all(
        ...
        if item["proposal_id"] != "FCF-FCP-0098"
    )
    decisions = {
    ...
    assert decisions["FCF-FCP-0098"] == "APPROVED"
