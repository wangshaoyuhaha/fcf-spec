...
from btc_finance_platform.paper_release_final_operator_receipt import build_paper_release_final_operator_receipt_closeout_checkpoint
from btc_finance_platform.paper_release_final_operator_receipt import build_paper_release_final_operator_receipt_export_packet
...
def test_p29_d7_final_operator_receipt_export_packet_is_local_static_read_only():
...
def test_p29_d8_final_operator_receipt_closeout_checkpoint_passes_completion_and_safety_gates():
    ...
    assert closeout["receipt_item_count"] == 15
    assert closeout["operator_review_required"] is True


def test_p29_d9_final_operator_receipt_handoff_packet_ready_for_operator_receipt_archive():
    handoff = build_paper_release_final_operator_receipt_handoff_packet()
    assert handoff["ok"] is True
    assert handoff["handoff_status"] == "READY_FOR_OPERATOR_RECEIPT_ARCHIVE"
    assert handoff["next_phase_candidate"] == "P30 Paper Release Final Delivery"
